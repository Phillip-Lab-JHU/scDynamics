...
# ==============================================================================
"""Calculates Anisotropic Motility features"""
import numpy as np
...
class DecomposedMotility3D(object):
    ...
    def plot_original_trajectories(self, path):
        draw_3D_trajectory(path, self.trajectories)

    def plot_rotated_trajectories(self, path):
        ...
        draw_3D_trajectory(path, registered_trajectories)
    ...
    def plot_msd_alpha(self, path):
        for traj_idx in self.msd_x:
            traj_msds_x = self.msd_x[traj_idx]
            traj_msds_y = self.msd_y[traj_idx]
            traj_msds_z = self.msd_z[traj_idx]

            self.draw_msd_alpha_each(path, traj_msds_x, traj_idx, axis='x')
            self.draw_msd_alpha_each(path, traj_msds_y, traj_idx, axis='y')
            self.draw_msd_alpha_each(path, traj_msds_z, traj_idx, axis='z')

    def draw_msd_alpha_each(self, path, traj_msds, traj_idx, axis='x'):
        log_tau = [np.log(tau) for tau in traj_msds.keys()]
        log_msd = [np.log(msd) for msd in traj_msds.values()]
        slope, intercept, r_val, p_val, SE = scipy.stats.linregress(log_tau, log_msd)
        ...
        if not os.path.isdir(path + 'msd_%s/' % axis):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
            os.makedirs(path + 'msd_%s/' % axis)
        plt.savefig(path + 'msd_%s/%s' % (axis, traj_idx))
        plt.clf()
        plt.close()
        ...
        if not os.path.isdir(path + 'msd_%s/' % axis):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
            os.makedirs(path + 'msd_%s/' % axis)
        plt.savefig(path + 'msd_%s/%s_log' % (axis, traj_idx))
        plt.clf()
        plt.close()
...
class DecomposedMotility2D(object):
    def __init__(self, trajectories, time_unit):
        ...
        self.avg_speed_x, self.max_speed_x, self.min_speed_x, self.net_distance_x, self.progressivity_x, self.speed_distribution_x, self.displacement_distribution_x, \
        self.avg_speed_y, self.max_speed_y, self.min_speed_y, self.net_distance_y, self.progressivity_y, self.speed_distribution_y, self.displacement_distribution_y, \
        self.exy_max,self.exy_total= self.get_various_distances(self.trajectories)

        self.msd_x, self.msd_y, self.alpha_x, self.alpha_y, = self.get_msd_alpha(self.trajectories)

        self.displ_variance_x, self.displ_cov_x, self.displ_skewness_x, self.displ_kurtosis_x, self.displ_ngaussalpha_x, self.displ_gini_x, \
        self.displ_variance_y, self.displ_cov_y, self.displ_skewness_y, self.displ_kurtosis_y, self.displ_ngaussalpha_y, self.displ_gini_y, = self.get_displ_distribution_props()

        self.displ_autocorr_x, self.displ_partial_autocorr_x, self.displ_autocorr_y, self.displ_partial_autocorr_y = self.get_displ_autocorr()

        #self.displ_hurst_RS_x, self.displ_hurst_RS_y,= self.get_displ_hurst_mandelbrot()
    ...
    def register_traj_disp_reflection(self, traj):
        max_dist, arg, tlag_max = -1, -1, -1
        for tlag in range(1, traj.shape[0]):
            dxyz = traj[tlag:] - traj[:-tlag]  # Displacement between two nearby points
            avg = np.ones((len(traj[:, 0]), 1)) * np.mean(traj, axis=0)
            ...
            x = xyr_r[:, 0]
            y = xyr_r[:, 1]
            list_dist = [self.calc_max_distance(x), self.calc_max_distance(y)]
            dist = max(list_dist)
            arg = np.argmax(list_dist)
            # print(tlag, calc_max_distance(x), calc_max_distance(y), calc_max_distance(z))
            if dist > max_dist:
        ...
        pc1_temp = rotated_traj[:, max_arg]
        pc2_temp = rotated_traj[:, max_arg2]

        rotated_traj = np.vstack((pc1_temp, pc2_temp)).T
        ...
        pc1, pc2 = rotated_traj_reflected[:, 0], rotated_traj_reflected[:, 1]

        return pc1, pc2, rotated_traj_reflected
    ...
    def plot_rotated_trajectories(self, path):
        ...
        for traj_idx in self.trajectories:  # u는 세포 하나의 index
            traj = self.trajectories[traj_idx]
            _, _, registered_traj = self.register_traj_disp_reflection(traj)
            registered_trajectories[traj_idx] = registered_traj
    ...
    def get_various_distances(self, trajectories):
        ...
        for traj_idx in trajectories:
            ...
            if max_displacement_x[traj_idx] == 0:
                exy_max[traj_idx] = np.nan
            elif max_displacement_x[traj_idx] != 0:
                exy_max[traj_idx] = max_displacement_y[traj_idx] / max_displacement_x[traj_idx]  # elongation parameters

            if total_distance_x[traj_idx] == 0:
                exy_total[traj_idx] = np.nan
            elif total_distance_x[traj_idx] != 0:
                exy_total[traj_idx] = total_distance_y[traj_idx] / total_distance_x[traj_idx]  # elongation parameters
    ...
    def get_msd_alpha(self, trajectories):
        ...
        return msds_x, msds_y, alphas_x, alphas_y
    ...
    def get_displ_distribution_props(self):
        ...
        for traj_idx in self.displacement_distribution_x:  # u 는 하나의 세포 index
            displ_variance_x[traj_idx], displ_cov_x[traj_idx], displ_skewness_x[traj_idx], \
            displ_kurtosis_x[traj_idx], displ_ngaussalpha_x[traj_idx], displ_gini_x[traj_idx] = self.calc_displ_distribution_props(self.displacement_distribution_x, traj_idx)

            displ_variance_y[traj_idx], displ_cov_y[traj_idx], displ_skewness_y[traj_idx], \
            displ_kurtosis_y[traj_idx], displ_ngaussalpha_y[traj_idx], displ_gini_y[traj_idx] = self.calc_displ_distribution_props(self.displacement_distribution_y, traj_idx)


        return displ_variance_x, displ_cov_x, displ_skewness_x, displ_kurtosis_x, displ_ngaussalpha_x, displ_gini_x, \
               displ_variance_y, displ_cov_y, displ_skewness_y, displ_kurtosis_y, displ_ngaussalpha_y, displ_gini_y

    def calc_displ_distribution_props(self, displacement_distribution, traj_idx):
        traj_displacement = np.array(displacement_distribution[traj_idx])
        if np.unique(traj_displacement).size == 1:  # Non-motile ( mostly [0, 0, 0, 0, 0, 0, ...])
            displ_variance = np.nan
            displ_cov = np.nan
            displ_skewness = np.nan
            displ_kurtosis = np.nan
            displ_ngaussalpha = np.nan
            displ_gini = np.nan
            ...
            displ_kurtosis = scipy.stats.kurtosis(traj_displacement)
            displ_ngaussalpha = np.mean(traj_displacement ** 4) / (3 * np.mean(traj_displacement ** 2) ** 2) - 1
            displ_gini = self.calc_gini(traj_displacement)

        return displ_variance, displ_cov, displ_skewness, displ_kurtosis, displ_ngaussalpha, displ_gini
    ...
    def get_displ_autocorr(self):
        ...
        return displ_autocorr_x, displ_partial_autocorr_x, displ_autocorr_y, displ_partial_autocorr_y
    ...
    def calc_displ_autocorr(self, displacement_distribution, traj_idx):
        ...
        if np.unique(traj_displacement).size == 1:  # Non-motile ( mostly [0, 0, 0, 0, 0, 0, ...])
            nlags = int(min(10 * np.log10(traj_displacement.size), traj_displacement.size - 1)) + 1
            # Default number of time lags for statsmodels.tsa.stattools.acf
            for i in range(1, nlags):
                traj_autocorr[i] = np.nan
            nlags = int(min(10 * np.log10(traj_displacement.size), traj_displacement.size // 2 - 1)) + 1
    ...
    def get_displ_hurst_mandelbrot(self):
        ...
        return displ_hurst_RS_x, displ_hurst_RS_y
    ...
    def extract_features(self, feature_list, tau_limit):
        ...
        dist_list = ['speed_distribution_x', 'speed_distribution_y']
        dict_temp = {}
        for feature_name in dist_list:
            feature_dict = getattr(self, feature_name)
            temp = []
