...
def verify_linking(df_linked, mask_path, mask_files, folder_name, image_num=10, img_format='tif'):
    ...
    #     #print(file_name)
    for t, mask_file in tqdm(enumerate(mask_files[:image_num]), total=len(mask_files[:image_num])):
        image = io.imread(mask_path + mask_file)
        ...
        if not os.path.isdir(mask_path + '%s/'%folder_name):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
            os.makedirs(mask_path + '%s/'%folder_name)
        image.save(mask_path + '%s/%s.%s' % (folder_name, t, img_format))