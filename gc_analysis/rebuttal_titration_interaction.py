...
path = r'\\philliplab-server.wse.jhu.edu\data\Chanhong\Cornell LN Spleen\Analysis\rebuttal\titration\\'
...
feature_list = df.columns[148:].drop(['PC1', 'PC2', 'Zone', 'kmeans'])
...
df2['type_dominance'].value_counts()
...
for feature in feature_list:
    dataset={}
    for condition in np.unique(df2['type_dominance']):
        data = df2[(df2['type_dominance'] == condition)][feature] # 'FDC_contact_times', 'FDC_contact_persistences'
    ...
    os.makedirs(path+'/interaction_feature_type_dominance_sc/', exist_ok=True)
    draw_custom_box_plot(dict_datasets, path+'interaction_feature_type_dominance_sc/',
                         file_name='%s'%feature,
...
# ----------------------------------------------------------------------
video_type_features['type_dominance'] = video_type_features['Type'].astype(str) + ' ' + video_type_features['Dominance'].astype(str)
video_type_features['type_dominance'].value_counts()
replace_keys = {'wt_B-cell WT-dominant':'WT / WT Dominant', 'mt_B-cell WT-dominant':'MT / WT Dominant',
                'wt_B-cell Comparable':'WT / Comparable', 'mt_B-cell Comparable':'MT / Comparable',
                'wt_B-cell MT-dominant':'WT / MT Dominant', 'mt_B-cell MT-dominant':'MT / MT Dominant', }
for feature in feature_list:
    dataset={}
    for condition in np.unique(video_type_features['type_dominance']):
        data = video_type_features[(video_type_features['type_dominance'] == condition)][feature] # 'FDC_contact_times', 'FDC_contact_persistences'
        dataset[condition] = np.array(data)
    new_order = ['wt_B-cell WT-dominant', 'mt_B-cell WT-dominant',
                 'wt_B-cell Comparable','mt_B-cell Comparable',
                 'wt_B-cell MT-dominant', 'mt_B-cell MT-dominant']
    ...
    os.makedirs(path+'/interaction_feature_type_dominance/', exist_ok=True)
    draw_custom_bar_plot(dict_datasets, path+'interaction_feature_type_dominance/',
                         file_name='%s'%feature,
...
for feature in feature_list:
    ...
    os.makedirs(path+'/interaction_feature_difference/', exist_ok=True)
    draw_custom_bar_plot(dict_datasets, path+'interaction_feature_difference/',
                             file_name='%s'%feature,
    ...
    plt.savefig(path+'interaction_feature_difference/' + 'diff %s regplot.png'%feature, dpi=300, bbox_inches='tight')
    os.makedirs(path+'interaction_feature_difference/' + 'svg/', exist_ok=True)
    plt.savefig(path+'interaction_feature_difference/' + 'svg/diff %s regplot.svg'%feature, bbox_inches='tight')
    plt.clf()
    plt.close()
# ----------------------------------------------------------------------
for feature in feature_list:
    ...
    os.makedirs(path + '/interaction_feature_mt_fraction/', exist_ok=True)
    plt.savefig(path + 'interaction_feature_mt_fraction/%s.png' % (feature), dpi=300, bbox_inches='tight')
    os.makedirs(path + 'svg/interaction_feature_mt_fraction/', exist_ok=True)
    plt.savefig(path + 'svg/interaction_feature_mt_fraction/%s.svg' % (feature), bbox_inches='tight')
    plt.clf()
    plt.close()
...
# Behavior-cluster assignments were generated in figure6.py from the same
# ordered GCB_no_inhibit dataframe and saved in Expanded_behavior.parquet.
behavior_path = (
    r'\\philliplab-server.wse.jhu.edu\data\Chanhong\Cornell LN Spleen'
    r'\Analysis\Figure6. Expanded behavior\\'
)
df_behavior = pd.read_parquet(
    behavior_path + 'Expanded_behavior.parquet',
    columns=['Type', 'beh_kmeans']
)

if len(df_behavior) != len(df2):
    raise ValueError(
        'Cannot assign beh_kmeans by row order: '
        f'Expanded_behavior has {len(df_behavior)} rows but df2 has {len(df2)} rows.'
    )

type_matches = (
    df_behavior['Type'].reset_index(drop=True)
    == df2['Type'].reset_index(drop=True)
)
if not type_matches.all():
    mismatch_rows = np.flatnonzero(~type_matches.to_numpy())
    raise ValueError(
        'Cannot assign beh_kmeans by row order because Type does not match '
        f'at {len(mismatch_rows)} rows. First mismatches: '
        f'{mismatch_rows[:10].tolist()}'
    )

# Expanded_behavior was generated from this same trajectory-level dataframe,
# so after confirming row count and Type order, copy behavior clusters by row.
df2['beh_kmeans'] = df_behavior['beh_kmeans'].to_numpy()

if df2['beh_kmeans'].isna().any():
    raise ValueError('Expanded_behavior.parquet contains missing beh_kmeans values')

all_mcs = sorted(df2['beh_kmeans'].unique())

mc_dist = (
    df2[df2['Type'].isin(['wt_B-cell', 'mt_B-cell'])]
    .groupby(['Video', 'Type', 'Dominance', 'mt_fraction', 'beh_kmeans'])
    .size()
    .rename('n')
    .reset_index()
)
...
mc_dist = (
    ...
    .apply(
        lambda g: g.set_index('beh_kmeans')
        .reindex(all_mcs)
        .rename_axis('beh_kmeans')
        .reset_index()
        .assign(
...
mc_profile = (
    mc_dist
    .groupby(['Type', 'Dominance', 'beh_kmeans'])['cluster_fraction']
    .mean()
    .reset_index()
)
...
mc_profile_wide = (
    mc_profile
    .pivot(index='Type_Dominance', columns='beh_kmeans', values='cluster_fraction')
    .reindex(columns=all_mcs, fill_value=0)
)

file_name = 'mt_dominance bc distribution'
vmax = 0.35
...
# Keep the labels generated by clustermap because they already follow the
# dendrogram's reordered rows.
...
plt.savefig(path+'behavior cluster correlation.png', dpi=300, bbox_inches='tight')

if not os.path.isdir(path + 'svg/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
    os.makedirs(path + 'svg/')
plt.savefig(path + 'svg/behavior cluster correlation.svg', bbox_inches='tight')
