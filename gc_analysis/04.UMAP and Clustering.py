...
df_features_all = pd.read_parquet(path+'all_features_%s_all.parquet'%duration)
df_duration_all = pd.read_parquet(path+'traj_duration_%s_all.parquet'%duration)
...
df_features_groupA = df_features[(df_features['Video'] == videos[1])|(df_features['Video'] == videos[2])|(df_features['Video'] == videos[4])|
                 (df_features['Video'] == videos[-1])].reset_index(drop=True)

df_duration_groupA = df_duration[(df_duration['Video'] == videos[1])|(df_duration['Video'] == videos[2])|(df_duration['Video'] == videos[4])
                 |(df_duration['Video'] == videos[-1])].reset_index(drop=True)

df_features_groupA = df_features_groupA[df_features_groupA['Type']!='T-cell'].reset_index(drop=True)
df_duration_groupA = df_duration_groupA[df_duration_groupA['Type']!='T-cell'].reset_index(drop=True)
...
motility_data_groupA = df_features_groupA.iloc[:,8:126].drop(['speed_distribution_x', 'speed_distribution_y', 'speed_distribution_z',],axis=1)
...
motility_data_scaled_groupA= pd.DataFrame(scaler.transform( motility_data_groupA ), columns=motility_data_groupA.columns)
...
pcs_groupA = pca.transform(motility_data_scaled_groupA)
...
kmeans_predicted_groupA = km.predict(pcs_groupA)
cluster_groupA = pd.DataFrame(kmeans_predicted_groupA, columns=['kmeans'])


from umap import UMAP
__umap = UMAP(metric='euclidean', n_components=2, n_neighbors=20, min_dist=0.1, random_state=0) # 40, 0.5
...
pcs_array_groupA = __umap.transform(pcs_groupA)
umap_groupA = pd.DataFrame(pcs_array_groupA, columns=['PC1', 'PC2'])
...
df_features_groupA['Type'] = 'WT-WT'
df_duration_groupA['Type'] = 'WT-WT'
df_groupA = pd.concat([df_features_groupA, umap_groupA, cluster_groupA], axis=1)
df_groupA = df_groupA.replace({'kmeans': replace_map})
df_with_groupA = pd.concat([df, df_groupA], axis=0).reset_index(drop=True)
df_duration_with_groupA = pd.concat([df_duration, df_duration_groupA], axis=0).reset_index(drop=True)


draw_umap_space(df, path, file_name='space_kmeans_ordered_', condition_name='kmeans',
                colors=cmc.batlow, dot_size=1, x_name='PC1', y_name='PC2')
draw_umap_space(df_with_inhibit, path, file_name='with inhibit space_kmeans_ordered', condition_name='Exp',
                colors=cmc.batlow, dot_size=1, x_name='PC1', y_name='PC2')
...
label_expanded = np.repeat(df_with_groupA['kmeans'], duration).reset_index(drop=True)
df_duration_with_groupA['kmeans'] = label_expanded
...
path = r'\\philliplab-server.wse.jhu.edu\data\Chanhong\Cornell LN Spleen\Analysis\feature_csvs\\'
df_with_groupA.to_parquet(path + 'GCB_with_groupA_all_features_%s.parquet'%duration)
df_with_groupA.to_csv(path + 'GCB_with_groupA_all_features_%s.csv'%duration, index=False)

df_duration_with_groupA.to_parquet(path + 'GCB_with_groupA_traj_duration_%s.parquet'%duration)
df_duration_with_groupA.to_csv(path + 'GCB_with_groupA_traj_duration_%s.csv'%duration, index=False)
