...
df_duration = pd.read_parquet(path+'long_traj_duration_20_all.parquet')
...
# Persistent-contact cutoff (frames) applied independently to each experiment.
# Change a treatment's value here without changing the quantification below.
min_duration_by_exp = {
    'Exp1': 15,
    'Exp2': 15,
    'Exp3': 15,
...
for cell_type in ['wt_B-cell', 'mt_B-cell']:
    ...
    for i in range(0, df_partial.shape[0]):
        if (i == 0) or (i == duration + i0):
            ...
            traj_exp = traj['Exp'].iloc[0]
            if traj_exp not in min_duration_by_exp:
                raise ValueError(
                    'No persistent-contact minimum duration is defined for '
                    f"Exp={traj_exp!r}. Add it to min_duration_by_exp."
                )
            current_min_duration = min_duration_by_exp[traj_exp]
            ...
            FDC_check_minduration = np.array([np.sum(element) for element in FDC_elements]) >= current_min_duration
            Tfh_check_minduration = np.array([np.sum(element) for element in Tfh_elements]) >= current_min_duration
            ...
            if (any(FDC_check_minduration) == False):
                ...
                df_PI_temp['min_duration'] = [current_min_duration]
...
for cell_type in np.unique(df_both_PI['Type']):
    ...
    df_part = df_part[df_part['Exp'] == 'mLT'].reset_index(drop=True)
...
draw_custom_bar_plot(dict_datasets, path, file_name='Tfh PC first or FDC PC first mLT',
                         strip_plot=True, colors = ('#888888', '#CC6677'), test='mann-whitney', pvalue=True, figsize=(1, 2),)
                     #vmin=vmin, vmax=vmax)

df_both_PI_ = df_both_PI.copy()
df_both_PI_['Exp'] = df_both_PI_['Exp'].replace({
    'Exp1': 'Ctrl',
    'Exp2': 'Ctrl',
    'Exp3': 'Ctrl',
})
print(df_both_PI_['Exp'].value_counts(dropna=False))
df_both_PI_ = df_both_PI_[df_both_PI_['Exp'] != 'IgG'].reset_index(drop=True)
df_both_PI_['type_exp'] = df_both_PI_['Type'].astype(str) + ' ' + df_both_PI_['Exp'].astype(str)

replace_keys = {'T-cell':'Tfh', 'mt_B-cell':'MT', 'wt_B-cell':'WT'}
dict_datasets_temp = {}
for cell_type in np.unique(df_both_PI_['type_exp']):
    df_part = df_both_PI_[(df_both_PI_['type_exp']==cell_type)].reset_index(drop=True)
    #df_part['FDC_duration']
    dict_datasets_temp[cell_type] = (df_part['Tfh_first'].values - df_part['FDC_first'].values) / 2

new_order = ['wt_B-cell Ctrl', 'wt_B-cell CD40L', 'wt_B-cell mLT', 'mt_B-cell Ctrl', 'mt_B-cell CD40L', 'mt_B-cell mLT']
ordered_dataset = change_dict_order(dict_datasets_temp, new_order)
dict_datasets = {replace_keys.get(k, k): v for (k, v) in ordered_dataset.items()}
vmin = np.min(flatten_nested_dict(dict_datasets))
vmax = np.max(flatten_nested_dict(dict_datasets))
draw_custom_bar_plot(dict_datasets, path, file_name='Tfh PC first or FDC PC first',
                         strip_plot=True, colors = ('#888888', '#888888', '#888888', '#CC6677', '#CC6677', '#CC6677',),
...
print(df_both_PI_['Exp'].value_counts(dropna=False))
df_both_PI_ = df_both_PI_[df_both_PI_['Exp'] != 'IgG'].reset_index(drop=True)
df_both_PI_ = df_both_PI_[df_both_PI_['Exp'] != 'CD40L'].reset_index(drop=True)
df_both_PI_['type_exp'] = df_both_PI_['Type'].astype(str) + ' ' + df_both_PI_['Exp'].astype(str)
...
new_order = ['wt_B-cell Ctrl', 'wt_B-cell mLT', 'mt_B-cell Ctrl', 'mt_B-cell mLT']
ordered_dataset = change_dict_order(dict_datasets_temp, new_order)
dict_datasets = {replace_keys.get(k, k): v for (k, v) in ordered_dataset.items()}
vmin = np.min(flatten_nested_dict(dict_datasets))
vmax = np.max(flatten_nested_dict(dict_datasets))
draw_custom_bar_plot(dict_datasets, path, file_name='Tfh PC first or FDC PC first Ctrl_and_mLT',
...
vmin = np.min(flatten_nested_dict(dict_datasets))
vmax = np.max(flatten_nested_dict(dict_datasets))
draw_custom_bar_plot(dict_datasets, path, file_name='dLZ Tfh PC first or FDC PC first mLT',
                         strip_plot=True, colors = ('#888888', '#CC6677'), test='mann-whitney', pvalue=True, figsize=(1, 2))
