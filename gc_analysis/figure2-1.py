...
# ==============================================================================
"""Generates Data for Figure2-1. General characterization of WT and MT GCB motility """
...
draw_jointplot(xs='PC1', y='PC2', df=df_, path=path, file_name='jointplot_type', hue="Type", colors=('#CC6677', '#888888', ), hue_order=['mt GCB', 'wt GCB'],
...
plt.xticks(fontsize=4, rotation=35, rotation_mode='anchor', ha='right', color='0.2', weight='bold')
plt.yticks(fontsize=4, rotation=0,  color='0.2', weight='bold')
...
for feature_name in feature_list:
    ...
    try:
        draw_custom_box_plot(dict_datasets, path+'int_feature_violin_plot_type/', file_name=feature_name, colors = ('#888888', '#CC6677'),
                                strip_plot=False, test='mann-whitney', pvalue=True, figsize=(1,2))
    except:
        pass
...
feature_list = ['quality_FDC_approach_times', 'quality_FDC_approach_persistences','quality_FDC_departure_times',
                ...
                'quality_LZ_departure_persistences','quality_LZ_stay_times', 'quality_LZ_stay_persistences',
                'quality_Core_approach_times', 'quality_Core_approach_persistences','quality_Core_departure_times',
                ...
                'DZ_distance_distance_slopes', 'DZ_distance_cov','DZ_distance_average', 'DZ_diff_distance_average',
                'LZ_distance_distance_slopes', 'LZ_distance_cov','LZ_distance_average', 'LZ_diff_distance_average',
...
df_long_ = df_long.replace({'Type': {'wt_B-cell': 'wt GCB', 'mt_B-cell': 'mt GCB'}})
...
draw_lineplot_by_custom_ranges(df_long_, path, folder_name='motility_feature_wrt_elapsed_time', feature_list=feature_list,
                                   condition_name='Type', custsom_range=(10, 30), stepsize=2, range_feature='Time_span',
...
df_long = pd.concat([df_basic, pd.DataFrame(label_list, columns=['Type', 'Time_span', 'Label'])], axis=1)
df_long['Time_span'] = df_long['Time_span'].astype(float) / 2  # Change frame -> min
df_long_ = df_long.replace({'Type': {'wt_B-cell': 'wt GCB', 'mt_B-cell': 'mt GCB', 'T-cell': 'Tfh'}})
df_long_.columns.get_loc('angle_partial_autocorr_3')
feature_list = df_long_.columns[:37]
...
for row, type in enumerate(['wt_B-cell', 'mt_B-cell']):
    ...
    ax.set_xlabel('Average speed (µm/min)', fontsize=12, weight='bold', color='0.2', labelpad=5)
