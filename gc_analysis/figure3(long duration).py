...
for feature_name in feature_list:
    ...
    rename_keys = {'wt_B-cell_NOI': 'WT NOI', 'wt_B-cell-FDC': 'WT PC',
                   'mt_B-cell_NOI': 'MT NOI','mt_B-cell-FDC': 'MT PC'}
...
####################################### T interaction frequency kde MT vs WT #############################################
...
ax.set_xlabel('T interaction frequency', fontsize=8, weight='normal', color='0.2')
...
####################################### Average T distance kde MT vs WT #############################################
replace_keys = {'mt_B-cell':'MT', 'wt_B-cell':'WT'}
...
ax.set_xlabel('Distance to Tfh (μm)', fontsize=8, weight='normal', color='0.2')
ax.set_ylabel('Density', fontsize=8, weight='normal', color='0.2')
plt.xticks(fontsize=8, color='0.2', weight='normal')
plt.yticks(fontsize=8, color='0.2', weight='normal')
...
rename_keys = {'wt_B-cell_far': 'WT far', 'wt_B-cell_close': 'WT close',
               'mt_B-cell_far': 'MT far', 'mt_B-cell_close': 'MT close',}
df_corr.rename(columns=rename_keys, inplace=True)
df_corr.rename(index=rename_keys, inplace=True)
...
for cell_type in ['mt_B-cell', 'wt_B-cell']:
    ...
    rename_keys = {'wt_B-cell_far': 'WT far',
                   'wt_B-cell_close': 'WT close', 'mt_B-cell_far': 'MT far', 'mt_B-cell_close': 'MT close', }
...
###################### Plot Close vs Far motility feature violin plot for MT and WT  ############################
...
for feature_name in feature_list:
    ...
    rename_keys = {'wt_B-cell_far': 'WT far', 'wt_B-cell_close': 'WT close', 'mt_B-cell_far': 'MT far', 'mt_B-cell_close': 'MT close'}
    dataset_renamed = {rename_keys.get(k, k): v for (k, v) in dataset.items()}
...
for feature_name in feature_list:
    ...
    ax.set_xlabel('Distance to Tfh (μm)', fontsize=16, weight='normal', color='0.2')
...
for feature_name in feature_list:
    ...
    ax.set_xlabel('Contact time with Tfh', fontsize=16, weight='normal', color='0.2')
    #ax.set_ylabel('Average speed (μm/min)', fontsize=16, weight='normal', color='0.2')
    plt.xticks(fontsize=16, color='0.2',weight='normal', )
    plt.yticks(fontsize=16, color='0.2', weight='normal', )
...
#################################### T cell, WT all interaction features wrt T distance ####################################
FDC_dist_range = (0,20)
feature_list = df.columns[324:].drop(['interacted_type'])
...
#################################### T cell, WT all interaction features wrt Tfh contact time ####################################
FDC_dist_range = (0,20)
feature_list = df.columns[324:].drop(['interacted_type'])
