...
df = pd.read_parquet(path+'GCB_with_groupA_all_features_20.parquet')
df_duration = pd.read_parquet(path+'GCB_with_groupA_traj_duration_20.parquet')
...
path = r'\\philliplab-server.wse.jhu.edu\data\Chanhong\Cornell LN Spleen\Analysis\Figure2-Supp. WT-WT\\'
...
df_ = df[df['Type']!='mt_B-cell'].reset_index(drop=True)
df_ = df_.replace({'Type': {'wt_B-cell': 'WT'}})

df_["Type"].value_counts()
#df_['kmeans'] = df['kmeans'].astype(str)

draw_umap_space(df_, path, file_name='space_Type', condition_name='Type', label_name='pseudo_Label', colors=('#CC6677', '#888888', '#6699CC'), x_name='PC1', y_name='PC2', dot_size=0.07)


draw_jointplot(xs='PC1', y='PC2', df=df_, path=path, file_name='jointplot_type', hue="Type", colors=('#888888', '#6699CC'), hue_order=['WT', 'WT-WT'],
...
df_ = df.copy()
df_ = df_[df_['Type']!='mt_B-cell'].reset_index(drop=True)
min_count = df_['Type'].value_counts().min()
sampled_df = df_.groupby('Type', group_keys=False).sample(n=min_count, random_state=42)
#sampled_df = sampled_df.replace({'Type': {'wt_B-cell': 'WT'}})
draw_cluster_distribution_heatmap(sampled_df, path, file_name='kmeans_type_heatmap_per_MC', condition_name='kmeans', cluster_type='Type',
                                  annot=True, col_cluster=False, row_cluster=False, transpose=True, vmax=80, cmap=cmc.oslo_r, figsize=(6,1.5))




entropies = {'WT-WT':[], 'wt_B-cell':[]}
for video in np.unique(df['Video']):
    df_part = df[df['Video']==video]
    df_part = df_part[df_part['Type'] != 'mt_B-cell'].reset_index(drop=True)
    entropy, max_entropy = calculate_entropy(df_part, df[df['Type']!='mt_B-cell'].reset_index(drop=True), condition_name='Type', cluster_type='kmeans')
    for type in entropy:
        entropies[type].append(entropy[type])

new_order = ['WT-WT', 'wt_B-cell']
...
for feature_name in feature_list:
    dataset={}
    for condition in np.unique(df_[condition_name]):
        data = df_[df_[condition_name] == condition][feature_name]
    ...
    draw_custom_violin_plot(dict_datasets, path+'feature_violin_plot_type/', file_name=feature_name, colors = colors,
                            test='mann-whitney', pvalue=True, figsize=(1,2))

    draw_custom_box_plot(dict_datasets, path+ 'feature_box_plot_type/', file_name=feature_name, colors = colors,
...
for feature_name in feature_list:
    dataset = {}
    for cell_type in np.unique(df_['Type']):
        df_part = df_[df_['Type'] == cell_type]
        avgs = []
        for video in videos:
...
for feature_name in feature_list:
    ...
    try:
        ...
        #                         strip_plot=False, test='mann-whitney', pvalue=True, figsize=(1,2))
        draw_custom_violin_plot(dict_datasets, path + 'int_feature_violin_plot_type/', file_name=feature_name,colors=('#888888', '#6699CC'),
                                test='mann-whitney', pvalue=True, figsize=(1, 2))
    ...
    draw_custom_bar_plot(dict_datasets, path + 'int_feature_bar_plot_type/', file_name=feature_name,
                         strip_plot=False, colors = ('#888888', '#6699CC') , test='mann-whitney', pvalue=True, figsize=(1, 2))
...
for idx, cluster in enumerate(np.unique(df_keyfeatures_scaled['kmeans'])):
    ...
    fig.update_layout(polar=dict(angularaxis=dict(showline=True, linecolor='black', linewidth=5, gridcolor='black')),plot_bgcolor='rgba(0,0,0,0)',legend=dict(
            x=1.55,  # x-coordinate of the legend
            y=1.2))
    fig.write_image(path + "kmeans_radarplot_%s.png"%cluster, format='png',engine='kaleido', scale=10,width=1000, height=1000, )
    if not os.path.isdir(path + 'svg/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
        os.makedirs(path + 'svg/')

    fig.write_image(path + 'svg/kmeans_radarplot_%s.svg' % cluster, format='svg', engine='kaleido' )
...
#################################### Box plot comparing all motility features by cell types ####################################
df_long_['Type'].value_counts()

if not os.path.isdir(path + 'long_int_feature_box/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
    os.makedirs(path + 'long_int_feature_box/')
if not os.path.isdir(path + 'long_int_feature_bar/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
    os.makedirs(path + 'long_int_feature_bar/')
if not os.path.isdir(path + 'long_int_feature_violin/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
    os.makedirs(path + 'long_int_feature_violin/')

#feature_list = ['FDC_contact_times', 'FDC_contact_persistences']
for feature_name in feature_list:
    dataset={}
    for condition in np.unique(df_long_[condition_name]):
        data = df_long_[df_long_[condition_name] == condition][feature_name]
        dataset[condition] = np.array(data)
    new_order = ['WT', 'MT']
    dict_datasets = change_dict_order(dataset, new_order)

    np.median(dict_datasets['WT'])
    np.median(dict_datasets['MT'])
    # draw_custom_box_plot(dict_datasets, path+ 'long_int_feature_box/', file_name=feature_name, colors = ('#888888', '#CC6677'),
    # strip_plot=False, test='t-test', pvalue=True, figsize=(1,2))
    #
    # draw_custom_bar_plot(dict_datasets, path + 'long_int_feature_bar/', file_name=feature_name,
    #                      colors=('#888888', '#CC6677'),
    #                      strip_plot=False, test='t-test', pvalue=True, figsize=(1, 2))

    draw_custom_violin_plot(dict_datasets, path + 'long_int_feature_violin/', file_name=feature_name,
                         colors=('#888888', '#CC6677'),  test='t-test', pvalue=True, figsize=(1, 2), vmax=70)
