...
feature_list = ['DZ_distance_average', 'Core_distance_average', 'LZ_distance_average', 'T_avg_overlap', 'T_contact_times', 'T_contact_persistences',
                'FDC_avg_overlap', 'FDC_contact_times', 'FDC_contact_persistences']
...
draw_lineplot_by_custom_ranges(df_, path, folder_name='int_feature_wrt_T_contacts', feature_list=feature_list,
                               condition_name='Type', custsom_range=(0, 16), stepsize=2, range_feature='T_contact_times',
                                   color_list=['#CC6677', '#888888'], marker_list=['o', '^', ], figsize=(4,4), x_label=xlabel,
                               estimator='mean', error_type='ci_norm', replace_keys=None, pvalue=True, legend=False, test='mann-whitney')
...
draw_lineplot_by_custom_ranges(df_, path, folder_name='int_feature_wrt_T_contact_persistence', feature_list=feature_list,
                               condition_name='Type', custsom_range=(0, 16), stepsize=2, range_feature='T_contact_persistences',
                                   color_list=['#CC6677', '#888888'], marker_list=['o', '^', ], figsize=(4,4), x_label=xlabel,
                               estimator='mean', error_type='ci_norm', replace_keys=None, pvalue=True, legend=False, test='mann-whitney')
...
draw_lineplot_by_custom_ranges(df_, path, folder_name='int_feature_wrt_FDC_contacts', feature_list=feature_list,
                               condition_name='Type', custsom_range=(0, 16), stepsize=2, range_feature='FDC_contact_times',
                                   color_list=['#CC6677', '#888888'], marker_list=['o', '^', ], figsize=(4,4), x_label=xlabel,
                               estimator='mean', error_type='ci_norm', replace_keys=None, pvalue=True, legend=False, test='mann-whitney')
...
draw_lineplot_by_custom_ranges(df_, path, folder_name='int_feature_wrt_FDC_persistence', feature_list=feature_list,
                               condition_name='Type', custsom_range=(0, 16), stepsize=2, range_feature='FDC_contact_persistences',
                                   color_list=['#CC6677', '#888888'], marker_list=['o', '^', ], figsize=(4,4), x_label=xlabel,
                               estimator='mean', error_type='ci_norm', replace_keys=None, pvalue=True, legend=False, test='mann-whitney')


xlabel = 'Number of Tfh Contacts'
draw_lineplot_by_custom_ranges(df_[df_['Zone_average'] >= 0.4], path, folder_name='nonDZ_int_feature_wrt_T_contacts', feature_list=feature_list,
                               condition_name='Type', custsom_range=(0, 16), stepsize=2, range_feature='T_contact_times',
                                   color_list=['#CC6677', '#888888'], marker_list=['o', '^', ], figsize=(4,4), x_label=xlabel,
                               estimator='mean', error_type='ci_norm', replace_keys=None, pvalue=True, legend=False, test='mann-whitney')

xlabel = 'Engaged volume fraction with FDC'
draw_lineplot_by_custom_ranges(df_, path, folder_name='int_feature_wrt_FDC_overlap', feature_list=feature_list,
                               condition_name='Type', custsom_range=(0, 0.3), stepsize=0.02, range_feature='FDC_avg_overlap',
                                   color_list=['#CC6677', '#888888'], marker_list=['o', '^', ], figsize=(4,4), x_label=xlabel,
                               estimator='mean', error_type='ci_norm', replace_keys=None, pvalue=True, legend=False, test='mann-whitney')


xlabel = 'Engaged volume fraction with Tfh'
draw_lineplot_by_custom_ranges(df_[df_['T_contact_times'] >= 10], path, folder_name='int_feature_wrt_T_overlap', feature_list=feature_list,
                               condition_name='Type', custsom_range=(0, 0.1), stepsize=0.01, range_feature='T_avg_overlap',
                                   color_list=['#CC6677', '#888888'], marker_list=['o', '^', ], figsize=(4,4), x_label=xlabel,
                               estimator='mean', error_type='ci_norm', replace_keys=None, pvalue=True, legend=False, test='mann-whitney')

#################################### Correlation between int features and speed ####################################
if not os.path.isdir(path + 'corr between int features/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
    os.makedirs(path + 'corr between int features/')


feature_list = ['quality_FDC_approach_persistences', 'quality_FDC_departure_persistences', 'quality_T_approach_persistences', 'quality_T_departure_persistences',
                'quality_DZ_approach_persistences', 'quality_DZ_departure_persistences', 'quality_LZ_approach_persistences', 'quality_LZ_departure_persistences',
                'quality_Core_approach_persistences', 'quality_Core_departure_persistences', 'FDC_distance_average', 'T_distance_average', 'DZ_distance_average',
                'LZ_distance_average', 'Core_distance_average', 'FDC_avg_overlap', 'FDC_contact_times', 'FDC_contact_persistences', 'T_avg_overlap',
                'T_contact_times', 'T_contact_persistences']
from itertools import combinations
for pair in combinations(feature_list, r=2):  # 2 for pairs, 3 for triplets, etc
    print(pair)
    ...
    for row, type in enumerate(['wt_B-cell', 'mt_B-cell']):
        ...
        sns.regplot(x=pair[0], y=pair[1], data=df_part_, scatter_kws={"color":"black", "alpha":0.3, 's':4}, line_kws={"color":"black"}, ax=ax)

        r, p = scipy.stats.pearsonr(df_part_[pair[0]], df_part_[pair[1]])
        ...
        ax.set_xlabel(pair[0], fontsize=12, weight='normal', color='0.2', labelpad=5)
        ax.set_ylabel(pair[1], fontsize=12, weight='normal', color='0.2', labelpad=5)
        #ax.set_xlim(16, 98)

    plt.savefig(path + 'corr between int features/%s_%s.png'%(pair[0], pair[1]), dpi=300, bbox_inches='tight')
    if not os.path.isdir(path + 'svg/corr between int features/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
        os.makedirs(path + 'svg/corr between int features/')
    plt.savefig(path + 'svg/corr between int features/%s_%s.svg'%(pair[0], pair[1]), bbox_inches='tight')
    plt.clf()
    plt.close()
...
draw_custom_violin_plot(dict_datasets, path, file_name='FDC persistent contacts',
...
dict_datasets_temp = {}
for cell_type in np.unique(df_both_PI['Type']):
    df_part = df_both_PI[(df_both_PI['Type']==cell_type)&(df_both_PI['Zone']>=1.6)].reset_index(drop=True)
    #df_part['FDC_duration']
    dict_datasets_temp[cell_type] = (df_part['Tfh_first'].values - df_part['FDC_first'].values) / 2
...
df_ = df_int.replace({'Type': {'wt_B-cell': 'WT', 'mt_B-cell': 'MT'}})
...
draw_jointplot(xs='int_PC1', y='int_PC2', df=df_, path=path, file_name='jointplot_type', hue="Type", colors=('#CC6677', '#888888', ), hue_order=['MT', 'WT'],
