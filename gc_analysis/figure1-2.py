...
# ==============================================================================
# ==============================================================================
"""Generates Data for Figure1-2. Behavior of T and wt GCB wrt FDC & LZ vs DZ"""
import scipy.stats
...
path = r'\\philliplab-server.wse.jhu.edu\data\Chanhong\Cornell LN Spleen\Analysis\Figure1. Dynamic Behavior of T and wt GCB\Zone dependent behavior\\'
...
####################################### FDC interaction kde Tfh vs wt GCB #############################################
...
####################################### Average FDC distance kde T cell vs wt GCB #############################################
replace_keys = {'T-cell':'Tfh', 'wt_B-cell':'wt GCB'}
...
for idx, coloc_feature in enumerate(coloc_features):
    ...
    ax = sns.kdeplot(data=dict_datasets['wt GCB'], fill=True, linewidth=1, clip=ranges[idx], color='#6699CC', label='wt GCB')
...
p_value_data = {}
for idx, cell_type in enumerate(['Tfh', 'wt GCB']):
    entropies = {'%s_DZ'%cell_type:entropies_dz[cell_type], '%s_DZ-sLZ'%cell_type:entropies_dz_slz[cell_type], '%s_sLZ'%cell_type:entropies_slz[cell_type],
                  '%s_sLZ-dLZ'%cell_type:entropies_slz_dlz[cell_type], '%s_dLZ'%cell_type:entropies_dlz[cell_type],}
...
p_values = []
pairs = []
for (mt_key, mt_values), (wt_keys, wt_values) in zip(p_value_data['Tfh'].items(), p_value_data['wt GCB'].items()):
    if test == 'mann-whitney':
...
######################## Zone interaction features Tfh vs wt GCB box plot  ###########################
...
# path = r'C:\Users\ChanhongMin\OneDrive - Johns Hopkins\Cornell LN Spleen\Analysis\Figure1. Dynamic Behavior of T and wt GCB\FDC dependent behavior\\'
#
# for cell_type in ['T-cell', 'wt_B-cell']:
#
#     fig, ax = plt.subplots(figsize=(2, 2))  # 2 inch by 2 inch
...
#                    'wt_B-cell_close': 'wt GCB close', 'T-cell_far': 'Tfh far', 'T-cell_close': 'Tfh close', }
#
#     entropies_ = {rename_keys.get(k, k): v for (k, v) in entropies.items()}
#     draw_custom_bar_plot(entropies_, path, file_name='entropy of far vs close for %s' %cell_type, colors=('#888888', '#CC6677'),
#                          strip_plot=True, test='mann-whitney', pvalue=True, figsize=(1,2))
#
# ###################### Plot Close vs Far motility feature violin plot for T cell and wt GCB  ############################
#
...
#
#     rename_keys = {'wt_B-cell_far': 'wt GCB far', 'wt_B-cell_close': 'wt GCB close', 'T-cell_far': 'Tfh far', 'T-cell_close': 'Tfh close'}
#     dataset_renamed = {rename_keys.get(k, k): v for (k, v) in dataset.items()}
#
...
#                             figsize=(2, 2))
#
# #################################### T cell, wt GCB avg speed wrt avg FDC distance ####################################
# FDC_dist_range = (0,20)
#
# mean_speed_dataset = {}
# std_speed_dataset = {}
...
#            capsize=3, capthick=1, elinewidth=1.5)
# ax.errorbar(range(FDC_dist_range[0], FDC_dist_range[1]+1), mean_speed_dataset['wt GCB'], 0.1*std_speed_dataset['wt GCB'], color='#6699CC',
#            capsize=3, capthick=1, elinewidth=1.5)
#
# for axis in ['bottom', 'left']:
#     ax.spines[axis].set_linewidth(2)
...
# ax.tick_params(width=2, color='0.2')
#
# ax.set_xlabel('Distance to FDC (μm)', fontsize=16, weight='bold', color='0.2')
...
# plt.close()
#
# #################################### T cell, wt GCB avg angle wrt avg FDC distance ####################################
#
# font = {'family': 'arial',
#             'weight': 'normal',
#             'size': 16}
...
#            capsize=3, capthick=1, elinewidth=1.5)
# ax.errorbar(range(FDC_dist_range[0], FDC_dist_range[1]+1), mean_angle_dataset['wt GCB'], 0.1*std_angle_dataset['wt GCB'], color='#6699CC',
#            capsize=3, capthick=1, elinewidth=1.5)
#
# for axis in ['bottom', 'left']:
#     ax.spines[axis].set_linewidth(2)
...
#
# ax.set_xlabel('Distance to FDC (μm)', fontsize=16, weight='bold', color='0.2')
# ax.set_ylabel('Average turning angle (rad/min)', fontsize=16, weight='bold', color='0.2')
# plt.xticks(fontsize=16, color='0.2',weight='bold', )
# plt.yticks(fontsize=16, color='0.2', weight='bold', )
#
# plt.legend(frameon=False, prop={'weight':'bold', 'size':12}, labelcolor='0.2')
...
#
#
# #################################### T cell, wt GCB all motility features wrt avg FDC distance ####################################
# FDC_dist_range = (0,20)
#
# feature_list = df.columns[128:283].drop(['speed_distribution', 'angle_distribution', 'speed_distribution_x', 'speed_distribution_y', 'speed_distribution_z'])
#
...
#                capsize=3, capthick=1, elinewidth=1.5)
#     ax.errorbar(range(FDC_dist_range[0], FDC_dist_range[1]+1), mean_dataset['wt GCB'], 0.1*std_dataset['wt GCB'], color='#6699CC',
#                capsize=3, capthick=1, elinewidth=1.5)
#
#     for axis in ['bottom', 'left']:
#         ax.spines[axis].set_linewidth(2)
...
#     plt.close()
#
# #################################### T cell, wt GCB all motility features wrt FDC contact time ####################################
# FDC_dist_range = (0,20)
#
# feature_list = df.columns[128:283].drop(['speed_distribution', 'angle_distribution', 'speed_distribution_x', 'speed_distribution_y', 'speed_distribution_z'])
#
...
#     plt.close()
#
# #################################### T cell, wt GCB all interaction features wrt FDC distance ####################################
# FDC_dist_range = (0,20)
...
#
#
# #################################### T cell, wt GCB all interaction features wrt FDC contact time ####################################
# FDC_dist_range = (0,20)
# feature_list = df.columns[324:]
#
# for feature_name in feature_list:
...
#
#
# #################################### T cell, wt GCB all features wrt avg FDC distance scatter plot ####################################
...
# ax.tick_params(width=2, color='0.2')
#
# ax.set_xlabel('Average turning angle (rad/min)', fontsize=16, weight='bold', color='0.2')
# ax.set_ylabel('B cell interaction score', fontsize=16, weight='bold', color='0.2')
# plt.xticks(fontsize=16, color='0.2',weight='bold', )
# plt.yticks(fontsize=16, color='0.2', weight='bold', )
#
# plt.legend(frameon=False, prop={'weight':'bold', 'size':12}, labelcolor='0.2')
...
# ax.tick_params(width=2, color='0.2')
#
# ax.set_xlabel('Average speed (um/min)', fontsize=16, weight='bold', color='0.2')
# ax.set_ylabel('B cell interaction score', fontsize=16, weight='bold', color='0.2')
# plt.xticks(fontsize=16, color='0.2',weight='bold', )
# plt.yticks(fontsize=16, color='0.2', weight='bold', )
#
