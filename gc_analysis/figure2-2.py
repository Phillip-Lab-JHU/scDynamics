...
# ==============================================================================
"""Generates Data for Figure2-2. FDC-dependent characterization of WT and MT motility """
import scipy.stats
...
####################################### Length scale of the GC (Distance to DZ from every zones) #############################################

replace_keys = {'mt_B-cell':'MT', 'wt_B-cell':'WT'}

dataset={}
for condition in ['DZ-sLZ', 'sLZ', 'sLZ-dLZ', 'dLZ']:
    data = df[(df['Zone'] == condition)]['DZ_distance_average'] # ['FDC_contact_times']
    dataset[condition] = np.array(data)
new_order = ['DZ-sLZ', 'sLZ', 'sLZ-dLZ', 'dLZ']
...
for i, key in enumerate(dict_datasets):
    plt.clf()  # Clear previous plot (resets axes)
    sns.kdeplot(dict_datasets[key], clip=(0.002, 70))
    line = plt.gca().lines[0]  # Get current axes, then the first line
    x, y = line.get_data()
    print(key, x[np.argmax(y)], y[np.argmax(y)])
    max_points[key] = (x[np.argmax(y)], y[np.argmax(y)])
...
#colors=('#BAC8DA', '#BEDCB0', '#4F609C', '#E9C61D', '#8A4F21')
colors=('#BEDCB0', '#4F609C', '#E9C61D', '#8A4F21')


fig, ax = plt.subplots(figsize=(2,2))
for i, key in enumerate(dict_datasets):
    ax = sns.kdeplot(data=dict_datasets[key], fill=True, linewidth=1, clip=(0, 70), color=colors[i], label=key)

    max_x, max_y = max_points[key][0], max_points[key][1]
    plt.scatter(max_x, max_y, s=1, zorder=2, color='red')  # zorder determines which image is in front (zorder = 2 is very front)
    plt.annotate('%s' % round(max_x, 2), xy=(max_x + 0.0002, max_y + 0.0002),
                 xytext=(max_x + 0.001, max_y + 0.015), fontsize=8, arrowprops=dict(arrowstyle='->', lw=0.5, color="red"))
    plt.plot([max_x, max_x], [0, max_y], color='0.2', linewidth=0.3, zorder=3, linestyle='--')
...
ax.set_xlabel('Distance to DZ (µm)', fontsize=8, weight='normal', color='0.2')
ax.set_ylabel('Density', fontsize=8, weight='normal', color='0.2')
plt.xticks(np.arange(0, 70 + 10, 10), fontsize=8, color='0.2', weight='normal')
plt.yticks(fontsize=8, color='0.2', weight='normal')

plt.legend(frameon=False, prop = {'weight':'normal', 'size':8}, labelcolor='0.2')

plt.savefig(path+'Distance to DZ frequency.png', dpi=300, bbox_inches='tight')

if not os.path.isdir(path + 'svg/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
    os.makedirs(path + 'svg/')
plt.savefig(path + 'svg/Distance to DZ frequency.svg', bbox_inches='tight')
plt.close()
plt.clf()


####################################### Length scale of the GC (Distance to dLZ from every zones) #############################################

replace_keys = {'mt_B-cell':'MT', 'wt_B-cell':'WT'}

dataset={}
for condition in ['DZ', 'DZ-sLZ', 'sLZ', 'sLZ-dLZ']:
    data = df[(df['Zone'] == condition)]['Core_distance_average'] # ['FDC_contact_times']
    dataset[condition] = np.array(data)
new_order = ['DZ', 'DZ-sLZ', 'sLZ', 'sLZ-dLZ']
ordered_dataset = change_dict_order(dataset, new_order)
dict_datasets = {replace_keys.get(k, k):v  for (k,v) in ordered_dataset.items() }
...
for i, key in enumerate(dict_datasets):
    plt.clf()  # Clear previous plot (resets axes)
    sns.kdeplot(dict_datasets[key], clip=(0.002, 100))
    line = plt.gca().lines[0]  # Get current axes, then the first line
    x, y = line.get_data()
    print(key, x[np.argmax(y)], y[np.argmax(y)])
    max_points[key] = (x[np.argmax(y)], y[np.argmax(y)])
...
#colors=('#BAC8DA', '#BEDCB0', '#4F609C', '#E9C61D', '#8A4F21')
colors=('#BAC8DA', '#BEDCB0', '#4F609C', '#E9C61D')
fig, ax = plt.subplots(figsize=(2,2))
for i, key in enumerate(dict_datasets):
    ax = sns.kdeplot(data=dict_datasets[key], fill=True, linewidth=1, clip=(0, 100), color=colors[i], label=key)
    ...
    if key != 'DZ':
        plt.text(max_x , max_y + 0.015, str(round(max_x, 2)), ha='center', va='top',
                 fontsize=8, fontdict={'weight': 'normal'}, color="black")

    else:
        plt.text(max_x + 0.001, max_y + 0.015, str(round(max_x, 2)), ha='left', va='top',
                 fontsize=8, fontdict={'weight': 'normal'}, color="black")


    # if key != 'DZ':
    #     # plt.annotate('%s' % round(max_x, 2), xy=(max_x + 0.0002, max_y + 0.0002),
    #     #              xytext=(max_x + 0.001, max_y + 0.015), fontsize=8,
    #     #              arrowprops=dict(arrowstyle='->', lw=0.5, color="red"))
    #
    #     plt.text(max_x + 0.001, max_y + 0.015, str(round(max_x, 2)), ha='left', va='top', transform=ax.transAxes,
    #              fontsize=12, fontdict={'weight': 'normal'}, color="black")
    #
    # else:
    #     plt.annotate('%s' % round(max_x, 2), xy=(max_x + 0.0005, max_y + 0.0005),
    #                  xytext=(max_x + 0.01, max_y + 0.015), fontsize=8,
    #                  arrowprops=dict(arrowstyle='->', lw=0.5, color="red"))
...
ax.set_xlabel('Distance to dLZ (µm)', fontsize=8, weight='normal', color='0.2')
ax.set_ylabel('Density', fontsize=8, weight='normal', color='0.2')
plt.xticks(np.arange(0, 100 + 10, 10), fontsize=8, color='0.2', weight='normal')
plt.yticks(fontsize=8, color='0.2', weight='normal')


plt.legend(frameon=False, prop = {'weight':'normal', 'size':8}, labelcolor='0.2')

plt.savefig(path+'Distance to dLZ frequency.png', dpi=300, bbox_inches='tight')

if not os.path.isdir(path + 'svg/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
    os.makedirs(path + 'svg/')
plt.savefig(path + 'svg/Distance to dLZ frequency.svg', bbox_inches='tight')
plt.close()
plt.clf()
...
normalized_count_dataset = {'WT DZ': [], 'MT DZ': [], 'WT DZ-sLZ': [], 'MT DZ-sLZ': [], 'WT sLZ': [], 'MT sLZ': [], 'WT sLZ-dLZ': [], 'MT sLZ-dLZ': [],
                            'WT dLZ': [], 'MT dLZ': []}
count_dataset = {'WT DZ': [], 'MT DZ': [], 'WT DZ-sLZ': [], 'MT DZ-sLZ': [], 'WT sLZ': [], 'MT sLZ': [], 'WT sLZ-dLZ': [], 'MT sLZ-dLZ': [],
                            'WT dLZ': [], 'MT dLZ': []}
partial_normalized_count_dataset = {'WT DZ': [], 'MT DZ': [], 'WT DZ-sLZ': [], 'MT DZ-sLZ': [], 'WT sLZ': [], 'MT sLZ': [], 'WT sLZ-dLZ': [], 'MT sLZ-dLZ': [],
                            'WT dLZ': [], 'MT dLZ': []}
...
colors = ('#888888', '#CC6677')*5
draw_custom_bar_plot(normalized_count_dataset, path, file_name='normalized count',
                     strip_plot=True, return_sig=True, colors=colors, test='wilcoxon-ranksum', pvalue=True, figsize=(4, 4))
...
df_.loc[(df_['Core_distance_average'] <= 16.58), 'dLZ_localization'] = 'dLZ'
df_.loc[(df_['Core_distance_average'] > 16.58), 'dLZ_localization'] = 'non-dLZ'

df_.loc[(df_['DZ_distance_average'] <= 13.02), 'DZ_localization'] = 'DZ'
df_.loc[(df_['DZ_distance_average'] > 13.02), 'DZ_localization'] = 'non-DZ'


normalized_count_dataset1 = {'WT non dLZ': [], 'MT non dLZ': [],  'WT dLZ': [], 'MT dLZ': []}
normalized_count_dataset2 = {'WT DZ': [], 'MT DZ': [],  'WT non DZ': [], 'MT non DZ': []}
...
draw_custom_bar_plot(normalized_count_dataset1, path, file_name='normalized dLZ localization count',
                     strip_plot=True, colors=colors, return_sig=True, test='wilcoxon-ranksum', pvalue=True, figsize=(2, 4))
...
####################################### dLZ FDC interaction kde MT vs WT #############################################
replace_keys = {'mt_B-cell':'MT', 'wt_B-cell':'WT'}
...
draw_custom_bar_plot(dict_datasets, path, file_name='dLZ cell-FDC interaction violinplot', colors = ('#888888', '#CC6677'), strip_plot=True,
...
dataset={}
for condition in np.unique(df['Type']):
    data = df[(df['Type'] == condition)&(df['Zone'] == 'sLZ')&(df['FDC_contact_times'] >=20)]['FDC_avg_overlap'] # ['FDC_contact_times']
    dataset[condition] = np.array(data)
    print(condition, np.array(data).shape[0])
new_order = ['wt_B-cell', 'mt_B-cell']
ordered_dataset = change_dict_order(dataset, new_order)
dict_datasets = {replace_keys.get(k, k):v  for (k,v) in ordered_dataset.items() }

draw_custom_violin_plot(dict_datasets, path, file_name='sLZ cell-FDC overlap violinplot', colors = ('#888888', '#CC6677'),
                            test='mann-whitney', pvalue=True, figsize=(1,2))
...
ax.set_xlabel('Engaged volume fraction', fontsize=8, weight='normal', color='0.2')
ax.set_ylabel('Density', fontsize=8, weight='normal', color='0.2')
plt.xticks(fontsize=8, color='0.2', weight='normal')
plt.yticks(fontsize=8, color='0.2', weight='normal')
...
####################################### Dotted (indicate length scales of GC) Average Zone distance kde MT vs WT #############################################
df_ = df.replace({'Type': {'wt_B-cell': 'WT', 'mt_B-cell': 'MT'}})
#df_ = df_[df_['Exp_group']=='C'].reset_index(drop=True)
...
for idx, coloc_feature in enumerate(coloc_features):
    ...
    if coloc_feature == 'DZ_distance_average':
        plt.axvline(2.97, color='0.2', linewidth=0.3, zorder=3, linestyle='--')
        plt.axvline(13.02, color='0.2', linewidth=0.3, zorder=3, linestyle='--')
        plt.axvline(27.79, color='0.2', linewidth=0.3, zorder=3, linestyle='--')
        plt.axvline(41.66, color='0.2', linewidth=0.3, zorder=3, linestyle='--')

    elif coloc_feature == 'Core_distance_average':
        plt.axvline(3.34, color='0.2', linewidth=0.3, zorder=3, linestyle='--')
        plt.axvline(16.58, color='0.2', linewidth=0.3, zorder=3, linestyle='--')
        plt.axvline(36.18, color='0.2', linewidth=0.3, zorder=3, linestyle='--')
        plt.axvline(45.73, color='0.2', linewidth=0.3, zorder=3, linestyle='--')
    ...
    legend = plt.legend(frameon=False, prop = {'weight':'normal', 'size':8}, labelcolor='0.2')
    legend.remove()
    plt.savefig(path+'dotted_%s.png'%coloc_feature, dpi=300, bbox_inches='tight')

    if not os.path.isdir(path + 'svg/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
        os.makedirs(path + 'svg/')
    plt.savefig(path + 'svg/dotted_%s.svg'%coloc_feature, bbox_inches='tight')
    plt.close()
    plt.clf()
...
draw_jointplot(xs=x_name, y=y_name, df=df_, path=path, file_name='speed vs FDC interaction jointplot',
               hue="Type", colors=('#CC6677', '#888888', ), hue_order=['MT', 'WT'],
...
new_order = [
    'WT DZ', 'WT DZ-sLZ', 'WT sLZ', 'WT sLZ-dLZ', 'WT dLZ',
    'MT DZ', 'MT DZ-sLZ', 'MT sLZ', 'MT sLZ-dLZ', 'MT dLZ'
]
...
df_corr_neworder = df_corr.loc[new_order, new_order]

mask = np.triu(df_corr_neworder) # Mask for only lower triangle

mask = mask[1:, :-1]  # mask that doesn't contain very top left and very bottom right block
corr = df_corr_neworder.iloc[1:,:-1].copy()  # df that doesn't contain very top left and very bottom right block
...
plt.savefig(path+'5 zone correlation.png', dpi=300, bbox_inches='tight')

if not os.path.isdir(path + 'svg/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
    os.makedirs(path + 'svg/')
plt.savefig(path + 'svg/5 zones correlation.svg', bbox_inches='tight')
...
group_df = draw_cluster_distribution_heatmap(df_, path, file_name='zone_heatmap', condition_name='Type', annot=False,
                                  cluster_type='Zone', col_cluster=False, figsize=(4,2))

p_dict = permutation_test(df_, group_name='Type', class_name='Zone', iteration=10000)
...
for zone in ['DZ', 'sLZ', 'dLZ']:
    ...
    for cluster in group_df.columns:
        ...
        diff = temp.loc['MT %s'%zone] - temp.loc['WT %s'%zone]
...
############# Shannon entropy of DZ vs sLZ vs dLZ for each video (Bar plot) ###############
df_ = df.copy()
df_['type_zone'] = df_['Type'].astype(str) + '_' + df_['Zone'].astype(str)

#entropies = {'mt_B-cell':[], 'wt_B-cell':[]}
entropies = {'wt_B-cell_DZ': [], 'wt_B-cell_DZ-sLZ': [], 'wt_B-cell_sLZ': [],
                   'wt_B-cell_sLZ-dLZ': [], 'wt_B-cell_dLZ': [],
                   'mt_B-cell_DZ': [], 'mt_B-cell_DZ-sLZ': [], 'mt_B-cell_sLZ': [],
                    'mt_B-cell_sLZ-dLZ': [], 'mt_B-cell_dLZ': []}

for video in np.unique(df['Video']):
    df_part = df_[df_['Video']==video]
    #counts = df_part['type_zone'].value_counts()
    #print(video, counts)
    # if np.min(counts) <=1:
    #     continue
    entropy, max_entropy = calculate_entropy(df_part, df_, condition_name='type_zone', cluster_type='kmeans')
    for type in entropy:
        entropies[type].append(entropy[type])

#entropies = {k: [x for x in v if x != 0] for k, v in entropies.items()} # Remove 0 in the values
for key, value in entropies.items():
    print(key, len(value))

replace_keys = {'wt_B-cell_DZ': 'WT DZ', 'wt_B-cell_DZ-sLZ': 'WT DZ-sLZ', 'wt_B-cell_sLZ': 'WT sLZ',
                   'wt_B-cell_sLZ-dLZ': 'WT sLZ-dLZ', 'wt_B-cell_dLZ': 'WT dLZ',
                   'mt_B-cell_DZ': 'MT DZ', 'mt_B-cell_DZ-sLZ': 'MT DZ-sLZ', 'mt_B-cell_sLZ': 'MT sLZ',
                    'mt_B-cell_sLZ-dLZ': 'MT sLZ-dLZ', 'mt_B-cell_dLZ': 'MT dLZ',}
entropies_ = {replace_keys.get(k, k):v  for (k,v) in entropies.items() }

dict_datasets = entropies_
file_name='entropy bar plot'
test = 'mann-whitney' # 'one-way anova_tukey', 'kruskal-wallis_dunn'

colors = ('#888888', '#888888', '#888888', '#888888', '#888888', '#CC6677', '#CC6677', '#CC6677', '#CC6677', '#CC6677',)
font = {'family': 'arial',
        'weight': 'normal',
        'size': 8}
matplotlib.rc('font', **font)
...
# category labels
plt.grid(False)
#pairs, p_values, cohen_ds = get_various_statistics(dict_datasets, test, return_sig=False)
...
target_pairs = [(0,1), (1,2), (2,3), (3,4), (5,6), (6,7), (7,8), (8,9),
                (0,5), (1,6), (2,7), (3,8), (4,9)]
target_indices = [pairs.index(pair) for pair in target_pairs]

p_value_subset = np.array(p_values)[target_indices]

#adjust_method = ['bonferroni', 'sidak', 'holm-sidak', 'holm', 'simes-hochberg', 'hommel', 'fdr_bh', 'fdr_by', 'fdr_tsbh', 'fdr_tsbky']
_, p_value_subset, _, _ = sm.stats.multipletests(p_value_subset, alpha=0.05,method='fdr_bh')  # Benjamini/Hochberg adjustment

cohen_ds_subset = np.array(cohen_ds)[target_indices]
plt.title('%s:%s:%s' % (target_pairs, p_value_subset, cohen_ds_subset))
...
############# Shannon entropy of 5 zones for each video (Line plot) ###############
...
############# Shannon entropy of DZ vs sLZ vs dLZ for each video (Line plot) ###############
...
plt.xticks(np.arange(len(list(mean_dataset))), ['DZ',  'sLZ', 'dLZ'], fontsize=16, rotation=35, # fontname = "Arial",
           rotation_mode='anchor', ha='right', color='0.2', weight='normal')
...
plt.title('%s:%s' % (pairs, p_values), fontsize=4)
plt.savefig(path + 'entropy of 3 zones.png', dpi=300, bbox_inches='tight')

if not os.path.isdir(path + 'svg/' ):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
    os.makedirs(path + 'svg/')
plt.savefig(path + 'svg/entropy of 3 zones.svg', bbox_inches='tight')
plt.clf()
plt.close()
...
###################### Plot Experimental int feature plots for Zones  ############################
df.columns.get_loc('quality_FDC_approach_times')
...
for idx in [0,1,2]:
    ...
    draw_lineplot_by_custom_ranges(df_, path, folder_name='motility_feature_wrt_%s'%coloc_feature, feature_list=feature_list,
                                   ...
                                   estimator='mean', error_type='ci_norm', fill=False, replace_keys=None, pvalue=True, test='mann-whitney', set_zero=False)
...
fig,axes = plt.subplots(nrows, ncols, figsize=(25, 10), sharey='row')
for row, type in enumerate(['wt_B-cell', 'mt_B-cell']):
    for col, zone in enumerate(['DZ', 'DZ-sLZ', 'sLZ', 'sLZ-dLZ', 'dLZ']):
        ax = axes[row][col]
        ...
        ax.set_xlabel('Average speed', fontsize=12, weight='normal', color='0.2', labelpad=5)
