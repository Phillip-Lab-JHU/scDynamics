...
############################### Visualize raw image ###############################
path = r'\\philliplab-server.wse.jhu.edu\data\Chanhong\Cornell LN Spleen\Raw images\Exp5\norm_img\\'
files = next(os.walk(path))[2]
bool_list = ['tif' in ele for ele in files]
files = np.array(files)[np.array(bool_list)]

tif = tifffile.TiffFile(path+files[-1])
...
# --- mLT files ---
# Good: 0, 1, 2, 12, 13, 14, 15, 16, 17, 18, 19
# OK: 3, 4
# 5, 6: No Green
# 7 : Bad
# 8, 9, 10, 11: Dense Tfh
