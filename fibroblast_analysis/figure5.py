...
"""Generates Data for Figure 5 old vs young."""
...
#
# path = r'\\philliplab-server.wse.jhu.edu\data\Charles\Motility\Low Density (25k cells)\0.5 Gel\analysis\Figure5. Classification\\'
...
df_duration.loc[(df_duration['Age'] <= 40), 'Type'] = 'Young'
df_duration.loc[(df_duration['Age'] > 40)&(df_duration['Age'] < 65), 'Type'] = 'Mid'
df_duration.loc[(df_duration['Age'] >= 65), 'Type'] = 'Old'

path = r'\\philliplab-server.wse.jhu.edu\data\Chanhong\Anshika motility project\figure4. prediction\\'

#################################### 3 class classification: Young vs Mid vs Old ####################################
...
le = LabelEncoder()
X_covar = le.fit_transform( np.array(df['Senescence']) )
...
X_traj_train, X_traj_test, X_covar_train, X_covar_test, y_train, y_test = train_test_split(X_traj, X_covar, y, test_size=0.2, random_state=0, stratify=y)
print(X_traj_train.shape, X_traj_test.shape, X_covar_train.shape, X_covar_test.shape, y_train.shape, y_test.shape)
...
from dnn.classification import scTRAIT_only_traj
sctrait = scTRAIT_only_traj(duration=new_duration, embed_dim=64, n_classes=np.unique(y).size)
result = sctrait.fit(x=[X_traj_train, X_covar_train], y=y_train, batch_size=256, epochs=500,
                     verbose=1, validation_split=0.1, shuffle=True, callbacks=[stop_early])
sctrait.save('saved_model/fibroblast_project/3 class scTRAIT', save_format='tf')
#3 class scTRAIT 2
...
vt.save('saved_model/fibroblast_project/3 class vanilla traj', save_format='tf')


#################################### Load trained model ####################################
sctrait = tf.keras.models.load_model('saved_model/fibroblast_project/3 class scTRAIT', compile=True)
# result = sctrait.fit(x=[X_traj_train, X_local_train, X_morph_train, X_covar_train], y=y_train, batch_size=256, epochs=2,
#                      verbose=1, validation_split=0.1, shuffle=True,)

vt = tf.keras.models.load_model('saved_model/fibroblast_project/3 class vanilla traj', compile=True)
...
y_pred = sctrait.predict([X_traj_test, X_covar_test])
...
dict_datasets = {'VT': np.array(acs2), 'scTRAIT': np.array(acs)}
#dict_datasets = {'scTRAIT': np.array(acs), 'Vanilla traj': np.array(acs2), 'Vanilla morpho': np.array(acs3),}
draw_custom_bar_plot(dict_datasets, path, file_name='accuracy scores', colors=('#CC6677', '#aa4499'), vmax=1,
                     strip_plot=False, test='t-test', pvalue=False, figsize=(1,2))


#################################### 2 class classification: Senescence vs Non senescence ####################################
...
from sklearn.preprocessing import LabelEncoder
le = LabelEncoder()
X_covar = le.fit_transform( np.array(df['Type']) )
le_name_mapping = dict(zip(le.classes_, le.transform(le.classes_)))
print(le_name_mapping)
...
result = sctrait.fit(x=[X_traj_train, X_covar_train], y=y_train, batch_size=256, epochs=500,
                     verbose=1, validation_split=0.1, shuffle=True, callbacks=[stop_early])
sctrait.save('saved_model/fibroblast_project/2 class scTRAIT', save_format='tf')
#3 class scTRAIT 2
### model2: Vanilla trajectory ###
vt = Temporal_Conv1D_2D_classifier(duration=new_duration, coor_dim=2, n_classes=np.unique(y).size)
result2 = vt.fit(x=X_traj_train, y=y_train, batch_size=256, epochs=500, verbose=1, validation_split=0.1, shuffle=True,
                     callbacks=[stop_early])
vt.save('saved_model/fibroblast_project/2 class vanilla traj', save_format='tf')

#################################### Load trained model ####################################
sctrait = tf.keras.models.load_model('saved_model/fibroblast_project/2 class scTRAIT', compile=True)
# result = sctrait.fit(x=[X_traj_train, X_local_train, X_morph_train, X_covar_train], y=y_train, batch_size=256, epochs=2,
#                      verbose=1, validation_split=0.1, shuffle=True,)

vt = tf.keras.models.load_model('saved_model/fibroblast_project/2 class vanilla traj', compile=True)
# result2 = vt.fit(x=X_traj_train, y=y_train, batch_size=256, epochs=2, verbose=1, validation_split=0.1, shuffle=True)
...
#################################### Model accuracy evaluation ####################################
draw_confusion_matrix(y_pred, y_test, y_names, path, figsize=(4,4), file_name='senescence scTRAIT confusion matrix', vmax=0.9)
draw_confusion_matrix(y_pred2, y_test, y_names, path, figsize=(4,4), file_name='senescence vanilla traj confusion matrix', vmax=0.9)
...
print(f1, f1_2, acs, acs2, ps, ps2, rs, rs2)

dict_datasets = {'VT': np.array(f1_2), 'scTRAIT': np.array(f1)}
#dict_datasets = {'scTRAIT': np.array(acs), 'Vanilla traj': np.array(acs2), 'Vanilla morpho': np.array(acs3),}
draw_custom_bar_plot(dict_datasets, path, file_name='f1 scores', colors=('#CC6677', '#aa4499'), vmax=1,
                     strip_plot=False, test='t-test', pvalue=False, figsize=(1,2))

dict_datasets = {'VT': np.array(ps2), 'scTRAIT': np.array(ps)}
#dict_datasets = {'scTRAIT': np.array(acs), 'Vanilla traj': np.array(acs2), 'Vanilla morpho': np.array(acs3),}
draw_custom_bar_plot(dict_datasets, path, file_name='precision scores', colors=('#CC6677', '#aa4499'), vmax=1,
                     strip_plot=False, test='t-test', pvalue=False, figsize=(1,2))

dict_datasets = {'VT': np.array(rs2), 'scTRAIT': np.array(rs)}
#dict_datasets = {'scTRAIT': np.array(acs), 'Vanilla traj': np.array(acs2), 'Vanilla morpho': np.array(acs3),}
draw_custom_bar_plot(dict_datasets, path, file_name='recall scores', colors=('#CC6677', '#aa4499'), vmax=1,
                     strip_plot=False, test='t-test', pvalue=False, figsize=(1,2))
...
df_rep['Type2'] = 'Young'
df_duration_rep['Type2'] = 'Young'
df_rep.loc[df_rep['Type'] != 'Young', 'Type2'] = 'Old'
df_duration_rep.loc[df_duration_rep['Type'] != 'Young', 'Type2'] = 'Old'

for patient in np.unique(df_rep['Patient']):
    print(patient, df_rep[df_rep['Patient']==patient]['Type'].reset_index(drop=True)[0])
...
#################################### Load trained model ####################################
sctrait = tf.keras.models.load_model('saved_model/monocyte_project_v2/young vs old scTRAIT', compile=True)
# result = sctrait.fit(x=[X_traj_train, X_local_train, X_morph_train, X_covar_train], y=y_train, batch_size=256, epochs=2,
#                      verbose=1, validation_split=0.1, shuffle=True,)


vt = tf.keras.models.load_model('saved_model/monocyte_project_v2/young vs old vanilla traj', compile=True)
# result2 = vt.fit(x=X_traj_train, y=y_train, batch_size=256, epochs=2, verbose=1, validation_split=0.1, shuffle=True)

vm = tf.keras.models.load_model('saved_model/monocyte_project_v2/young vs old vanilla morpho', compile=True)
# result3 = vm.fit(x=X_morph_train, y=y_train, batch_size=256, epochs=2, verbose=1, validation_split=0.1, shuffle=True)


sctrait = tf.keras.models.load_model('saved_model/monocyte_project_v2/3 class scTRAIT', compile=True)
vt = tf.keras.models.load_model('saved_model/monocyte_project_v2/3 class vanilla traj', compile=True)
vm = tf.keras.models.load_model('saved_model/monocyte_project_v2/3 class vanilla morpho', compile=True)

# patients = np.unique(df['Patient'])
# for patient in patients:
#     sctrait = tf.keras.models.load_model('saved_model/monocyte_project_v2/leave one out young vs old scTRAIT %s'%patient, compile=True)
...
#
#     draw_confusion_matrix(y_pred_rep, y_rep, y_names_rep, path, figsize=(4,4), file_name='no %s scTRAIT confusion matrix'%patient, vmax=0.9)
#     draw_confusion_matrix(y_pred_rep2, y_rep, y_names_rep, path, figsize=(4,4), file_name='no %s vanilla traj confusion matrix'%patient, vmax=0.9)
#     draw_confusion_matrix(y_pred_rep3, y_rep, y_names_rep, path, figsize=(4,4), file_name='no %s vanilla morph confusion matrix'%patient, vmax=0.9)

#################################### prediction ####################################
path = r'\\philliplab-server.wse.jhu.edu\data\Charles\Motility\Low Density (25k cells)\0.5 Gel\analysis_v2\Figure5. Classification\rep old vs young\\'
#path = r'\\philliplab-server.wse.jhu.edu\data\Charles\Motility\Low Density (25k cells)\0.5 Gel\analysis_v2\Figure5. Classification\rep 3 class\\'
...
#     print(thresh)
#     draw_confusion_matrix(y_pred_rep, y_rep, y_names_rep, path, figsize=(4,4), file_name='rep scTRAIT confusion matrix %s thresh'%thresh, vmax=0.9, thresh=thresh)
...
# pd.unique(y_names)
# #print(le_name_mapping)
# df_traj.loc[df_traj['Type'] == '0', 'Type'] = 'Old'
# df_traj.loc[df_traj['Type'] == '1', 'Type'] = 'Young'
#
# df_local['Label'] = 'Data'
...
# pd.unique(y_names)
# #print(le_name_mapping)
# df_local.loc[df_local['Type'] == '0', 'Type'] = 'Old'
# df_local.loc[df_local['Type'] == '1', 'Type'] = 'Young'
#
# df_morph['Label'] = 'Data'
...
# pd.unique(y_names)
# #print(le_name_mapping)
# df_morph.loc[df_morph['Type'] == '0', 'Type'] = 'Old'
# df_morph.loc[df_morph['Type'] == '1', 'Type'] = 'Young'
#
# path = r'\\philliplab-server.wse.jhu.edu\data\Charles\Motility\Low Density (25k cells)\0.5 Gel\analysis_v2\Figure5. Classification\old vs young\\'
#
...
#                     height=6, aspect=1, hue='Type', hue_order=['Old', 'Young'], palette=('#beaed4', '#7fc97f'), diag_sharey=False, despine=True, corner=True,)
...
#
# for idx, name in enumerate(['traj_emb', 'local_emb', 'morph_emb']):
#
#     fig, ax = plt.subplots(figsize=(2, 2))
#     x_name = name+'_PC1'
#     y_name = name+'_PC2'
