...
for i in np.unique(df['Age']):
    print(i, df[df['Age']==i].shape[0])


path = r'\\philliplab-server.wse.jhu.edu\data\Chanhong\Anshika motility project\figure1. holistic space\\'
...
draw_umap_space(df, path, file_name='motility space_type', condition_name='Type', label_name='pseudo_label',
                colors = ('#F2B342', '#B56478', '#2983B1'), dot_size=0.07, x_name='PC1', y_name='PC2')
draw_umap_space(df, path, file_name='motility space_age', condition_name='Age', label_name='pseudo_label',
                colors =cmc.turku, dot_size=0.07, x_name='PC1', y_name='PC2')
draw_umap_space(df, path, file_name='motility space_senescence', condition_name='Senescence', label_name='pseudo_label',
                colors =('#888888', '#B589C3'), dot_size=0.07, x_name='PC1', y_name='PC2')
draw_umap_space(df, path, file_name='motility space_kmeans', condition_name='kmeans', label_name='pseudo_label',
                colors=cmc.batlow, dot_size=0.07, x_name='PC1', y_name='PC2')
draw_umap_space(df, path, file_name='motility space_gender', condition_name='Gender', label_name='pseudo_label',
                colors=('#6699CC', '#44AA99'), dot_size=0.07, x_name='PC1', y_name='PC2')
...
draw_jointplot(xs='PC1', y='PC2', df=df, path=path, file_name='jointplot_type', hue="Type", colors=('#F2B342', '#B56478', '#2983B1'),
               legend=False, fill=False, thresh=0.3, height=4, ratio=5, space=0, xlabels='UMAP1', ylabel='UMAP2',
               xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax)
draw_jointplot(xs='PC1', y='PC2', df=df, path=path, file_name='jointplot_age', hue="Age", colors=cmc.turku,
               legend=False, fill=False, thresh=0.6, height=4, ratio=5, space=0, xlabels='UMAP1', ylabel='UMAP2',
               xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax)
draw_jointplot(xs='PC1', y='PC2', df=df, path=path, file_name='jointplot_senescence', hue="Senescence", colors=('#888888', '#B589C3'),
               legend=False, fill=False, thresh=0.3, height=4, ratio=5, space=0, xlabels='UMAP1', ylabel='UMAP2',
               xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax)
draw_jointplot(xs='PC1', y='PC2', df=df, path=path, file_name='jointplot_kmeans', hue="kmeans", colors=cmc.batlow,
               ...
               xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax)
draw_jointplot(xs='PC1', y='PC2', df=df, path=path, file_name='jointplot_gender', hue="Gender", colors=('#6699CC', '#44AA99'),
               legend=False, fill=False, thresh=0.3, height=4, ratio=5, space=0, xlabels='UMAP1', ylabel='UMAP2',
               xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax)
...
draw_space_feature_magnitude(df, path, feature_list=['avg_speed'], dot_size=0.07, x_name='PC1', y_name='PC2', vmax=4)
draw_space_feature_magnitude(df, path, feature_list=['displ_cov'], dot_size=0.07, x_name='PC1', y_name='PC2', vmax=4)
draw_space_feature_magnitude(df, path, feature_list=['angle_cov'], dot_size=0.07, x_name='PC1', y_name='PC2', vmax=2)
...
draw_relative_cluster_distribution_heatmap(df, path, file_name='relative_kmeans_type_heatmap', condition_name='Type', cluster_type='kmeans',
                                  annot=True, transpose=False, col_cluster=False, row_cluster=True, cmap=cmc.oslo_r, figsize=(6,2))


draw_heatmap_with_circles(df, path, file_name='kmeans_type_circleheatmap', condition_name='Type', cluster_type='kmeans',
                          vmax=None, transpose=False, row_cluster=True, col_cluster=False, figsize=(4,4))


draw_cluster_distribution_heatmap(df, path, file_name='kmeans_senescence_heatmap', condition_name='Senescence', cluster_type='kmeans',
                                  annot=True, col_cluster=False, row_cluster=True, cmap=cmc.bilbao_r, figsize=(6,2))

draw_relative_cluster_distribution_heatmap(df, path, file_name='relative_senescence_type_heatmap', condition_name='Senescence', cluster_type='kmeans',
                                  annot=True, transpose=False, col_cluster=False, row_cluster=True, cmap=cmc.oslo_r, figsize=(6,2))


draw_cluster_distribution_heatmap(df, path, file_name='all_cell_line_kmeans_heatmap', condition_name='Cell Line',
                                  cluster_type='kmeans', transpose=False, row_cluster=True, col_cluster=False, cmap=cmc.bilbao_r, figsize=(4,12))

draw_relative_cluster_distribution_heatmap(df, path, file_name='relative_all_cell_line_kmeans_heatmap', condition_name='Cell Line',
                                           cluster_type='kmeans', transpose=False, row_cluster=False, col_cluster=False,
                                           cmap=cmc.oslo_r, figsize=(4,12))


draw_cluster_distribution_heatmap(df, path, file_name='all_age_kmeans_heatmap', condition_name='Age',
                                  cluster_type='kmeans', transpose=True, row_cluster=False, col_cluster=False, cmap=cmc.bilbao_r, figsize=(12,4))
...
#################################### Patient-wise entropy young vs mid vs old ####################################

entropies = {'Young':[], 'Mid':[], 'Old':[]}
for video in np.unique(df['Cell Line']):
    df_part = df[df['Cell Line']==video]
...
#################################### Patient-wise entropy nonsen vs sen ####################################
...
#################################### Cross correlation of young vs mid vs old ###################################
...
for feature_name in feature_list:
    ...
    draw_custom_violin_plot(dataset, path+'motility_feature_violin_plot_type/', file_name=feature_name, colors = ('#2983B1', '#F2B342', '#B56478'),
                            test='kruskal-wallis_dunn', pvalue=True, figsize=(1,2))

    draw_custom_box_plot(dataset, path+ 'motility_feature_box_plot_type/', file_name=feature_name, colors = ('#2983B1', '#F2B342', '#B56478'),
...
#replace_keys = {'T-cell':'Tfh', 'wt_B-cell':'wt GCB'}
if not os.path.isdir(path + 'motility_feature_violin_plot_senescence/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
    os.makedirs(path + 'motility_feature_violin_plot_senescence/')

if not os.path.isdir(path + 'motility_feature_box_plot_senescence/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
    os.makedirs(path + 'motility_feature_box_plot_senescence/')
...
for feature_name in feature_list:
    ...
    draw_custom_violin_plot(dataset, path+'motility_feature_violin_plot_senescence/', file_name=feature_name, colors = ('#888888', '#B589C3'),
                            test='mann-whitney', pvalue=True, figsize=(1,2))

    draw_custom_box_plot(dataset, path+ 'motility_feature_box_plot_senescence/', file_name=feature_name, colors = ('#888888', '#B589C3'),
...
condition_name = 'Type'
batch_name = 'Cell Line'
batches = np.unique(df[batch_name])

colors = ('#2983B1', '#F2B342', '#B56478')

if not os.path.isdir(path + 'experimental_motility_feature_type/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
    os.makedirs(path + 'experimental_motility_feature_type/')
...
for feature_name in feature_list:
    ...
    draw_custom_bar_plot(dataset, path+'experimental_motility_feature_type/', file_name=feature_name,
...
if not os.path.isdir(path + 'experimental_motility_feature_senescence/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
    os.makedirs(path + 'experimental_motility_feature_senescence/')
...
for feature_name in feature_list:
    ...
    draw_custom_bar_plot(dataset, path+'experimental_motility_feature_senescence/', file_name=feature_name,
...
df.columns.get_loc('inst_angle_cosine_similarity_entropies')
feature_list = list( df.columns[8:103].drop(['speed_distribution_x', 'speed_distribution_y']) )
from sklearn.preprocessing import StandardScaler  # (x-mu)/sigma
...
#################################### Z scores of filtered motility features wrt kmeans ####################################

feature_list = ['avg_speed', 'displ_cov', 'alphas', 'progressivity', 'angle_cov', 'displ_autocorr_1', 'angle_autocorr_1', 'exy_total', 'avg_angle']
from sklearn.preprocessing import StandardScaler  # (x-mu)/sigma
...
g.ax_heatmap.set_xticklabels(g.ax_heatmap.get_xmajorticklabels(), fontsize=16, rotation=0, rotation_mode='anchor', ha='center')
g.ax_heatmap.set_yticklabels(g.ax_heatmap.get_ymajorticklabels(), fontsize=16, va='center')

x0, _y0, _w, _h = g.cbar_pos
g.ax_cbar.set_position([0.9, 1, g.ax_row_dendrogram.get_position().width*5, 0.02])
...
plt.savefig(path+'kmeans Z score filtered features_heatmap.png', dpi=300, bbox_inches='tight')
if not os.path.isdir(path + 'svg/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
    os.makedirs(path + 'svg/')
plt.savefig(path + 'svg/' + 'kmeans Z score filtered features_heatmap.svg', bbox_inches='tight',)
plt.clf()
plt.close()
...
feature_list = df.columns[2:103].drop(['phi', 'speed_distribution_x', 'speed_distribution_y'])
...
draw_volcano_plot(df_p, path, file_name='volcano plot of %s vs %s'%(ref, compare), z_thresh=0.3, p_thresh=50, z_name='AvgZ', p_name='Adj_Logp',
