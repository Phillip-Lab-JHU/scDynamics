...
#################################### Training dataset ####################################
path = r'\\philliplab-server.wse.jhu.edu\data\Charles\Motility\Low Density (25k cells)\0.5 Gel\analysis\Figure6. Age score\\'
df = pd.read_parquet(path+'df.parquet')
df_duration = pd.read_parquet(path+'df_duration.parquet')
...
sctrait.save('saved_model/monocyte_project/regression interpretation young_old scTRAIT', save_format='tf')

#################################### Get predicted cellular age ####################################
from tensorflow.keras import models
sctrait = tf.keras.models.load_model('saved_model/monocyte_project/regression interpretation young_old scTRAIT', compile=True)
...
path = r'\\philliplab-server.wse.jhu.edu\data\Charles\Motility\Low Density (25k cells)\0.5 Gel\analysis\Figure7. Interpretation_youngold\\'
...
sns.kdeplot(x='Age', y='pred', data=df, alpha=1, hue='Type',hue_order=['Young', 'Old'], ax=ax,
            zorder=1, linewidths=1.5, palette =('#7fc97f', '#beaed4'), fill=False,  common_norm=False, thresh=0.1,)


file_name = 'Age Regression'

#ax.plot(age_range, age_range, linestyle='--', color='black',  linewidth=1 )
old = df[df['Type']=='Old'].reset_index(drop=True)
young = df[df['Type']=='Young'].reset_index(drop=True)
...
sns.regplot(x='Age', y='pred', data=young, scatter=False, #scatter_kws={"color":"black", "alpha":0.7, 's':10},
            line_kws={"color":"red", 'linewidth':1.5}, ax=ax)

mae = np.mean([ abs(i - j) for i, j in zip(young['Age'], young['pred']) ])
r, p = scipy.stats.pearsonr(young['Age'], young['pred'])

plt.text(0.1, 0.95,"MAE = %s years" %str(round(mae, 2)), ha='left', va='top', transform=ax.transAxes,
                         fontsize=14, fontdict={'weight': 'bold'}, color="#7fc97f")
...
df.columns.get_loc('pre_morph_PC_34')

df_corr = df.iloc[:, 191:304]
corr = df_corr.corr(method='pearson')
...
#df_corr1 = pd.concat([df_motility_pc, df_morpho_pc, df_colocalize_pc], axis=1)
    ...
    df_corr2 = df.loc[:, ['pre_traj_PC_0', 'pre_traj_PC_1', 'pre_local_PC_0', 'pre_local_PC_1', 'pre_morph_PC_0','pre_morph_PC_1']]
...
#corr = df_corr1.corrwith(df_corr2, axis=0)
    ...
    g=sns.clustermap(corr, annot=False, cmap=cmc.cork, col_cluster=False, row_cluster=True,
    #cbar_pos=(1, 0.2, 0.03, 0.8),
    metric='correlation', method='average',
    linewidths=0.5, linecolor='black',
    alpha=1,
...
df_corr2 = df.loc[:, ['post_traj_PC_0', 'post_traj_PC_1', 'post_traj_PC_2', 'post_local_PC_0', 'post_local_PC_1', 'post_local_PC_2',
                      'post_morph_PC_0','post_morph_PC_1', 'post_morph_PC_2', 'post_morph_PC_3']]
...
df.columns.get_loc('post_traj_PC_0')
df.columns.get_loc('pre_morph_PC_34')
