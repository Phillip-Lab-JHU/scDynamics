...
class BasicMotility(object):
    ...
    def plot_3D_trajectories(self, path):
    ...
    def get_various_distances(self):
        ...
        for traj_idx in self.trajectories:  # u는 세포 하나의 index
            ...
            if total_distance[traj_idx] == 0:
                progressivity[traj_idx] = np.nan
            elif total_distance[traj_idx] != 0:
    ...
    def get_various_angles(self):
        ...
        for traj_idx in self.trajectories:  # u는 세포 하나의 index
            ...
            avg_angle[traj_idx] = np.mean(angle_list) / self.time_unit
            max_angle[traj_idx] = max(angle_list) / self.time_unit
            min_angle[traj_idx] = min(angle_list) / self.time_unit
    ...
    def get_msd_alpha(self):
        ...
        for traj_idx in self.trajectories:
            ...
            while tau <= max_tau:
                ...
                tau += 1
            if len(traj_msds) == 0:
                msds[traj_idx] = np.nan
                alphas[traj_idx] = np.nan
    ...
    def plot_msd_alpha(self, path):
        for traj_idx in self.msd:
            ...
            if not os.path.isdir(path + 'msd/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
                os.makedirs(path + 'msd/')
            plt.savefig(path + 'msd/%s' % traj_idx)
            plt.clf()
            plt.close()
            ...
            if not os.path.isdir(path + 'msd/'):  # Returns Boolean (if UMAP_fig folder doesn't exist, False)
                os.makedirs(path + 'msd/')
            plt.savefig(path + 'msd/%s_log' % traj_idx)
            plt.clf()
            plt.close()
    ...
    def get_displ_distribution_props(self):
        ...
        for traj_idx in self.displacement_distribution:  # u 는 하나의 세포 index
            ...
            if np.unique(traj_displacement).size == 1:  # Non-motile ( mostly [0, 0, 0, 0, 0, 0, ...])
                displ_variance[traj_idx] = np.nan
                displ_cov[traj_idx] = np.nan
                displ_skewness[traj_idx] = np.nan
                displ_kurtosis[traj_idx] = np.nan
                displ_ngaussalpha[traj_idx] = np.nan
                displ_gini[traj_idx] = np.nan
            else:
    ...
    def get_angle_distribution_props(self):
        ...
        for traj_idx in self.angle_distribution:  # u 는 하나의 세포 index
            ...
            if np.unique(traj_angle).size == 1:  # Non-motile / Perfect directed motion ( mostly [0, 0, 0, 0, 0, 0, ...])
                angle_variance[traj_idx] = np.nan
                angle_cov[traj_idx] = np.nan
                angle_skewness[traj_idx] = np.nan
                angle_kurtosis[traj_idx] = np.nan
                angle_ngaussalpha[traj_idx] = np.nan
                angle_gini[traj_idx] = np.nan
            else:
    ...
    def get_angle_autocorr(self):
        ...
        for traj_idx in self.angle_distribution:
            ...
            traj_partial_autocorr = {}
            if np.unique(traj_angle).size == 1:  # Non-motile / Perfect directed motion ( mostly [0, 0, 0, 0, 0, 0, ...])
                nlags = int(min(10*np.log10(traj_angle.size), traj_angle.size-1))+1
                # Default number of time lags for statsmodels.tsa.stattools.acf
                for i in range(1, nlags):
                    traj_autocorr[i] = np.nan
                nlags = int(min(10 * np.log10(traj_angle.size), traj_angle.size//2 - 1)) + 1
                # Default number of time lags for statsmodels.tsa.stattools.pacf
                for i in range(1, nlags):
                    traj_partial_autocorr[i] = np.nan
