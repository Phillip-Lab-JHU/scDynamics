...
# df_ctrl = df_nonfrail[df_nonfrail['Condition']=='Control'].reset_index(drop=True)
# df_dna = df_nonfrail[df_nonfrail['Condition']=='DNA'].reset_index(drop=True)
# df_il6 = df_nonfrail[df_nonfrail['Condition']=='IL6'].reset_index(drop=True)
# df_lps = df_nonfrail[df_nonfrail['Condition']=='LPS'].reset_index(drop=True)

#################################### 3 class classification ####################################

#################################### Training data preparation ####################################

path = r'\\philliplab-server.wse.jhu.edu\data\Charles\Motility\Low Density (25k cells)\0.5 Gel\analysis_v2\Figure5. Classification\3 class\\'
...
sctrait.save('saved_model/monocyte_project_v2/3 class scTRAIT', save_format='tf')
#3 class scTRAIT 2
...
vt.save('saved_model/monocyte_project_v2/3 class vanilla traj', save_format='tf')
...
result3 = vm.fit(x=X_morph_train, y=y_train, batch_size=256, epochs=500, verbose=1, validation_split=0.1, shuffle=True,
                     callbacks=[stop_early])
vm.save('saved_model/monocyte_project_v2/3 class vanilla morpho', save_format='tf')
...
#################################### 2 class classification: Old vs Young ####################################
...
result = sctrait.fit(x=[X_traj_train, X_local_train, X_morph_train, X_covar_train], y=y_train, batch_size=256, epochs=500,
                     verbose=1, validation_split=0.1, shuffle=True, callbacks=[stop_early])
sctrait.save('saved_model/monocyte_project_v2/young vs old scTRAIT', save_format='tf')

### model2: Vanilla trajectory ###
vt = Temporal_Conv1D_2D_classifier(duration=new_duration, coor_dim=2, n_classes=np.unique(y).size)
result2 = vt.fit(x=X_traj_train, y=y_train, batch_size=256, epochs=500, verbose=1, validation_split=0.1, shuffle=True,
                     callbacks=[stop_early])
vt.save('saved_model/monocyte_project_v2/young vs old vanilla traj', save_format='tf')

### model3: Vanilla morphology ###
vm = Temporal_Conv1D_2D_classifier(duration=new_duration, coor_dim=8, n_classes=np.unique(y).size)
result3 = vm.fit(x=X_morph_train, y=y_train, batch_size=256, epochs=500, verbose=1, validation_split=0.1, shuffle=True,
                     callbacks=[stop_early])
vm.save('saved_model/monocyte_project_v2/young vs old vanilla morpho', save_format='tf')
