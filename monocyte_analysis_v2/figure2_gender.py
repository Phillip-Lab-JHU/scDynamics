...
path = r'\\philliplab-server.wse.jhu.edu\data\Charles\Motility\Low Density (25k cells)\0.5 Gel\analysis_v2\Figure2. Perturbation_gender\\'
...
draw_umap_space(df_young, young_path, file_name='motility space_media_young', condition_name='Sex', label_name='pseudo_particle',
                colors = ('#DDCC77', '#88CCEE'), dot_size=0.07, x_name='PC1', y_name='PC2')

draw_jointplot(xs='PC1', y='PC2', df=df_young, path=young_path, file_name='jointplot_condition', hue="Sex", colors=('#DDCC77', '#88CCEE'),
               hue_order=['Female', 'Male'], legend=False, fill=True, thresh=0.2, height=4, ratio=5, space=0, xlabels='UMAP1', ylabel='UMAP2',
               xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax)

########################################################################

draw_umap_space(df_old, old_path, file_name='motility space_media_old', condition_name='Sex', label_name='pseudo_particle',
                colors = ('#DDCC77', '#88CCEE'), dot_size=0.07, x_name='PC1', y_name='PC2')
draw_jointplot(xs='PC1', y='PC2', df=df_old, path=old_path, file_name='jointplot_condition', hue="Sex", colors=('#DDCC77', '#88CCEE'),
               hue_order=['Female', 'Male'], legend=False, fill=True, thresh=0.2, height=4, ratio=5, space=0, xlabels='UMAP1', ylabel='UMAP2',
               xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax)

########################################################################
draw_umap_space(df_frail, frail_path, file_name='motility space_media_frail', condition_name='Sex', label_name='pseudo_particle',
                colors = ('#DDCC77', '#88CCEE'), dot_size=0.07, x_name='PC1', y_name='PC2')

draw_jointplot(xs='PC1', y='PC2', df=df_frail, path=frail_path, file_name='jointplot_condition', hue="Sex", colors=('#DDCC77', '#88CCEE'),
               hue_order=['Female', 'Male'], legend=False, fill=True, thresh=0.2, height=4, ratio=5, space=0, xlabels='UMAP1', ylabel='UMAP2',
               xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax)


for cond in np.unique(df['Condition']):
    draw_umap_space(df[df['Condition']==cond].reset_index(drop=True), path, file_name='%s_space_type'%cond, condition_name='Sex', label_name='pseudo_particle',
                    colors = ('#DDCC77', '#88CCEE'), dot_size=0.07, x_name='PC1', y_name='PC2')
    draw_jointplot(xs='PC1', y='PC2', df=df[df['Condition']==cond].reset_index(drop=True), path=path, file_name='%s_jointplot_type'%cond, hue="Sex",
                   colors=('#DDCC77', '#88CCEE'), alpha=0.6, legend=False, fill=True,
...
draw_cluster_distribution_heatmap(df_young, young_path, file_name='Sex_kmeans_young_heatmap', col_cluster=False, row_cluster=False,
                                  annot=False, vmax=20, cmap=cmc.bilbao_r, condition_name='Sex', cluster_type='kmeans', figsize=(4,2))
draw_cluster_distribution_heatmap(df_old, old_path, file_name='Sex_kmeans_old_heatmap', col_cluster=False, row_cluster=False,
                                  annot=False, vmax=20, cmap=cmc.bilbao_r, condition_name='Sex', cluster_type='kmeans', figsize=(4,2))
draw_cluster_distribution_heatmap(df_frail, frail_path, file_name='Sex_kmeans_frail_heatmap', col_cluster=False, row_cluster=False,
                                  annot=False, vmax=20, cmap=cmc.bilbao_r, condition_name='Sex', cluster_type='kmeans', figsize=(4,2))

draw_relative_cluster_distribution_heatmap(df_young, young_path, file_name='relative_Sex_kmeans_young_heatmap', col_cluster=False, row_cluster=False,
                                  annot=False, vmax=70, cmap=cmc.oslo_r, transpose=False, condition_name='Sex', cluster_type='kmeans', figsize=(4,2))
draw_relative_cluster_distribution_heatmap(df_old, old_path, file_name='relative_Sex_kmeans_old_heatmap', col_cluster=False, row_cluster=False,
                                  annot=False, vmax=70,cmap=cmc.oslo_r, transpose=False, condition_name='Sex', cluster_type='kmeans', figsize=(4,2))
draw_relative_cluster_distribution_heatmap(df_frail, frail_path, file_name='relative_Sex_kmeans_frail_heatmap', col_cluster=False, row_cluster=False,
                                  annot=False, vmax=70, cmap=cmc.oslo_r, transpose=False, condition_name='Sex', cluster_type='kmeans', figsize=(4,2))
...
for feature_name in feature_list:
    ...
    for condition in np.unique(df_young[condition_name]):
        for sex in np.unique(df_young['Sex']):
            data = df_young[(df_young[condition_name] == condition)&(df_young['Sex'] == sex)][feature_name]
            dataset[condition+' '+sex] = np.array(data)
    ...
    draw_custom_violin_plot(dataset, young_path+'motility_feature_violin_plot_type/', file_name=feature_name,
                            colors = ('#888888', '#888888', '#CC6677', '#CC6677', '#44AA99', '#44AA99', '#6699CC', '#6699CC'), test=test, pvalue=True, figsize=(4,2))

    draw_custom_bar_plot(dataset, young_path + 'motility_feature_box_plot_type/', file_name=feature_name, vmax=vmax,
                         strip_plot=False, colors=('#888888', '#888888', '#CC6677', '#CC6677', '#44AA99', '#44AA99', '#6699CC', '#6699CC'), test=test, pvalue=True, figsize=(4, 2))
...
for feature_name in feature_list:
    ...
    for condition in np.unique(df_old[condition_name]):
        for sex in np.unique(df_old['Sex']):
            data = df_old[(df_old[condition_name] == condition) & (df_old['Sex'] == sex)][feature_name]
            dataset[condition + ' ' + sex] = np.array(data)
    # new_order = ['wt_B-cell', 'T-cell']
    # ordered_dataset = change_dict_order(dataset, new_order)
    # dict_datasets = {replace_keys.get(k, k):v  for (k,v) in ordered_dataset.items() }

    draw_custom_violin_plot(dataset, old_path + 'motility_feature_violin_plot_type_old/', file_name=feature_name,
                            colors=('#888888', '#888888', '#CC6677', '#CC6677', '#44AA99', '#44AA99', '#6699CC', '#6699CC'),
                            test=test, pvalue=True, figsize=(4, 2))

    draw_custom_bar_plot(dataset, old_path + 'motility_feature_box_plot_type_old/', file_name=feature_name, vmax=vmax,
                         strip_plot=False, colors=('#888888', '#888888', '#CC6677', '#CC6677', '#44AA99', '#44AA99', '#6699CC', '#6699CC'), test=test, pvalue=True,
                         figsize=(4, 2))
...
for feature_name in feature_list:
    ...
    for condition in np.unique(df_frail[condition_name]):
        for sex in np.unique(df_frail['Sex']):
            data = df_frail[(df_frail[condition_name] == condition) & (df_frail['Sex'] == sex)][feature_name]
            dataset[condition + ' ' + sex] = np.array(data)
    # new_order = ['wt_B-cell', 'T-cell']
    # ordered_dataset = change_dict_order(dataset, new_order)
    # dict_datasets = {replace_keys.get(k, k):v  for (k,v) in ordered_dataset.items() }

    draw_custom_violin_plot(dataset, frail_path + 'motility_feature_violin_plot_type_frail/', file_name=feature_name,
                            colors=('#888888', '#888888', '#CC6677', '#CC6677', '#44AA99', '#44AA99', '#6699CC', '#6699CC'),
                            test=test, pvalue=True, figsize=(4, 2))

    draw_custom_bar_plot(dataset, frail_path + 'motility_feature_box_plot_type_frail/', file_name=feature_name, vmax=vmax,
                         strip_plot=False, colors=('#888888', '#888888', '#CC6677', '#CC6677', '#44AA99', '#44AA99', '#6699CC', '#6699CC'),
                         test=test, pvalue=True,figsize=(4, 2))
...
df_ = df.copy()
df_['type_condition'] = df['Type'].astype(str) + ' ' + df['Condition'] + ' ' + df['Sex']
draw_cluster_distribution_heatmap(df_, path, file_name='all_kmeans_heatmap', condition_name='type_condition', vmax=30,
                                  cluster_type='kmeans', col_cluster=False, row_cluster=True, cmap=cmc.bilbao_r, figsize=(4,7))
